...
// ── 8. 集計が実際の回答と合う ──────────────────────────
section("8. 結果は、取りに行った人にだけ出る（D112 / D134）");
{
  // --- 他人には出ない -------------------------------------------------------
  //
  // **D112 は「他人の作品ページ＝絵とタイトルだけ」と決めている。**
  // 以前はここで誰にでも伝達率が見えていた。仕様の取りこぼしだった。
  const res = await guesser.get(`/works/${workId}`);
  const t = textOf(res.html);

  must(!/項目別の伝達率/.test(t), "他人には伝達率が出ていない（D112）");
  must(!/読み解きました/.test(t), "他人には予告も出ていない");

  // --- 作者にも、開くまでは数字が出ない -------------------------------------
  const sealed = await author.get(`/works/${workId}`);
  const st = textOf(sealed.html);

  must(/2人が、あなたの絵を読み解きました/.test(st), "作者には人数の予告が出る");
  // 片方が全問不正解なので、まったく違う読まれ方が1人
  must(/うち1人は、まったく違うものを見ていました/.test(st), "違いの予告が出る");
  must(!/%/.test(st), "**開く前に % が出ていない**（予告が数字にならない）");
  must(/開く/.test(st), "開くボタンがある");

  // --- 開くと出る -----------------------------------------------------------
  const opened = await author.get(`/works/${workId}?result=open`);
  const ot = textOf(opened.html);

  must(/50%/.test(ot), "開くと正答率が出る");
  must(/項目別の伝達率/.test(ot), "開くと項目別も出る");
  must(/代わりに選ばれたもの/.test(ot), "誤答の中身が出る");
  must(/誰が選んだかは記録していません/.test(ot), "回答者の身元は出さないと書いてある");
  ...
  // 数字が消えたのではなく、作品ページで取りに行けば見える。
  const list = await guesser.get("/works");
  must(
    !/回答 \d/.test(textOf(list.html)),
    "一覧のカードに回答数が出ていない（D112）",
  );
}
