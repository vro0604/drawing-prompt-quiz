 ...
 *   1-b. 古い作業木から本番を変えられない柵の自己試験
 *   1-c. main への push を捕まえる hook の自己試験（実際に push して確かめる）
...
const STEPS = [
  ...
  // 柵は2つある。「本番へつながない」（上）と「古い作業木から本番を変えない」（下）。
  // どちらも効いていないと、後続は全部「本番を壊しうる状態」で走る。
  { key: "preflight", name: "古い作業木から本番を変えられない柵の自己試験", cmd: "npm", args: ["run", "test:preflight"] },
  // 手で `git push origin <枝>:main` と打つ穴を塞げているかは、判定だけでは分からない。
  // 使い捨ての repo へ実際に push して、止まることを見る。
  { key: "preflight-hook", name: "main への push を捕まえる hook の自己試験", cmd: "npm", args: ["run", "test:preflight:hook"] },
