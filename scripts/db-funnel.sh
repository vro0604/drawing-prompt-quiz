#!/usr/bin/env bash
#
# db-funnel.sh ／ 初回利用の進みぐあいを、パスワードを手入力せずに本番から読む
#
# db-audit-prod.sh と同じ手順で接続文字列を組み立て、
# **読むだけの集計**（scripts/db-funnel.mjs）にだけ渡す。
#
# この入口は本番へつなぐ。ローカル試験（test:db / test:e2e / test:smoke:local）は
# 別の入口で、そちらには本番向きの環境変数を見つけたら止まる柵が入っている。
# 2つを混ぜないために、コマンドを分けてある。
#
# 使い方: npm run db:funnel [-- --period 7d] [-- --json]
...
set +e
SUPABASE_DB_URL="$url" node scripts/db-funnel.mjs "$@"
status=$?
set -e
