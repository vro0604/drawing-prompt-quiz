...
export const RANKING_TYPES: {
  ...
  //   単体表示なら弱く生まれるだけだが、**並べ替えられると序列が自動生成される。**
  //   伝達率は絵の伝わりやすさを測る軸なので、順位にすると
  //   D105 の「下手なままでいい」という証明が、そのまま上手さの物差しに変わる。
  //
  //   人気（いいね順）は残す。**上手さの序列ではない**から。
  //   これを残すので D7（匿名のいいね廃止）の根拠は消えず、C3 にも波及しない。
  //
  //   RPC 側（get_rankings）はまだ accuracy を受け付ける。
  //   画面から選べないだけで害は無いので、SQL の掃除は別の単位でやる。
  //   ?type=accuracy が来ても resolveRankingType が人気に落とす。
  {
    value: "duration",
    label: "時間別",
