...
const NOTE =
  "このページは開発用です。見ているのは「Supabase に届くか」だけで、テーブルの中身は見ていません。";

/**
 * Supabase との接続を確認するための診断ページ（開発用）。
 *
 * 「**わざと存在しないテーブル**を問い合わせて、返ってきたエラーの種類で判定する」
 * という方法をとる。
 *
 ...
 *
 * 【テーブルはもう有るのに、まだこの方法を使っている】
 *   この書きかたは Step 3（テーブルを1つも作っていない時期）に置いたもので、
 *   **本物のクエリに置き換える判断は、まだ出していない。**
 *   置き換えると「届くか」と「その表を読む権限があるか」が1つの結果に混ざるため、
 *   何を見せる画面にするかを決める必要がある。BACKLOG.md の B-002 に置いた。
 *
 *   `__connection_check__` は、実在しないことを名前で示すための名前。
 *   同じ名前のテーブルを作ると、この判定は静かに壊れる。
 */
