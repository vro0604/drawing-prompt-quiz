...
supabase/migrations/  72本<!--count:migration-->。スキーマと関数の全部
...
| `npm run test:db` | **仕様どおりに動くか。**お題の組み立て・持ち出し・制作時間・挑戦の時計・フレーバー・漏洩・回帰・次の作品の配給を、実際に呼んで確かめる | 475項目<!--count:縦断試験--> |
...
| `npm run test:e2e` | **画面が本当に動くか。**Chromium を起動して、人と同じ順に押す。帯の秒・時間切れ・持ち出し・返歌・次の作品・一覧の絞り込み・スマホ幅まで | 187項目<!--count:ブラウザ試験--> |
| `npm run test:e2e:support` | **支援画面を確認する。**390px・1440px の表示、金額入力、Founder 導線と完了画面を Chromium で見る | PC・スマホ |
