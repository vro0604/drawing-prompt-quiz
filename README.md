...
supabase/migrations/  66本<!--count:migration-->。スキーマと関数の全部
                      （うち64本が本番へ入っている。残る2本は課金 v0
                      20260917090000 / 100000 で、本番へは未適用。
                      本番の64本のうち D194・D195・D196 の3本
                      20260912110000 / 120000 / 130000 は 2026-09-16 に1本ずつ適用。
                      本番の履歴は64本・最大 20260914120000）
...
| `npm run db:verify:local` | **同じ検査を、本番に触れずに。**Postgres を WebAssembly で起動し、migration を全部当ててから流す | 224項目<!--count:DB構造の検査--> |
| `npm run test:db` | **仕様どおりに動くか。**お題の組み立て・持ち出し・制作時間・挑戦の時計・フレーバー・漏洩・回帰・次の作品の配給を、実際に呼んで確かめる | 419項目<!--count:縦断試験--> |
