...
supabase/migrations/  61本<!--count:migration-->。スキーマと関数の全部
...
| `npm run db:verify:local` | **同じ検査を、本番に触れずに。**Postgres を WebAssembly で起動し、migration を全部当ててから流す | 198項目<!--count:DB構造の検査--> |
| `npm run test:db` | **仕様どおりに動くか。**お題の組み立て・持ち出し・制作時間・挑戦の時計・フレーバー・漏洩・回帰・次の作品の配給を、実際に呼んで確かめる | 346項目<!--count:縦断試験--> |
