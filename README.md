...
supabase/migrations/  64本<!--count:migration-->。スキーマと関数の全部
                      （本番の61本と同じもの＋本番へ未適用の3本（D194・D195・D196。
                      版番号 20260912110000 / 120000 / 130000）。課金 v0 は入っていない。
                      2026-09-16 の実測で、本番の履歴は61本・最大 20260914120000。
...
| `npm run db:verify:local` | **同じ検査を、本番に触れずに。**Postgres を WebAssembly で起動し、migration を全部当ててから流す | 211項目<!--count:DB構造の検査--> |
| `npm run test:db` | **仕様どおりに動くか。**お題の組み立て・持ち出し・制作時間・挑戦の時計・フレーバー・漏洩・回帰・次の作品の配給を、実際に呼んで確かめる | 386項目<!--count:縦断試験--> |
