...
supabase/migrations/  62本<!--count:migration-->。スキーマと関数の全部
                      （本番の59本と同じもの＋本番へ未適用の3本（D194・D195・D196）。
                      課金 v0 は入っていない。本番の履歴は59本（2026-09-11 の
                      読み取り監査）。この作業木は onboarding-phase12-integration）
...
| `npm run test:db` | **仕様どおりに動くか。**お題の組み立て・持ち出し・制作時間・挑戦の時計・フレーバー・漏洩・回帰・次の作品の配給を、実際に呼んで確かめる | 383項目<!--count:縦断試験--> |
...
| `npm run test:tools` | **本番へ書く道具が、当てる前に断るべきものを断るか。**名前の取り違え・重複・当てる先の食い違いを断ること、途中で落ちたら1本も残らないことを試す | 27項目<!--count:道具の自己試験--> |
