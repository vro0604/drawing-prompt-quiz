...
| `npm run db:verify:local` | **同じ検査を、本番に触れずに。**Postgres を WebAssembly で起動し、migration を全部当ててから流す | 179項目<!--count:DB構造の検査--> |
| `npm run test:db` | **仕様どおりに動くか。**お題の組み立て・持ち出し・制作時間・挑戦の時計・フレーバー・漏洩・回帰・次の作品の配給を、実際に呼んで確かめる | 269項目<!--count:縦断試験--> |
...
| `npm run test:e2e` | **画面が本当に動くか。**Chromium を起動して、人と同じ順に押す。帯の秒・時間切れ・持ち出し・返歌・次の作品・一覧の絞り込み・スマホ幅まで | 106項目<!--count:ブラウザ試験--> |
