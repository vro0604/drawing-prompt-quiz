...
supabase/migrations/  67本<!--count:migration-->。スキーマと関数の全部
...
| `npm run db:verify:local` | **同じ検査を、本番に触れずに。**Postgres を WebAssembly で起動し、migration を全部当ててから流す | 225項目<!--count:DB構造の検査--> |
| `npm run test:db` | **仕様どおりに動くか。**お題の組み立て・持ち出し・制作時間・挑戦の時計・フレーバー・漏洩・回帰・次の作品の配給を、実際に呼んで確かめる | 424項目<!--count:縦断試験--> |
...
| `npm run test:e2e` | **画面が本当に動くか。**Chromium を起動して、人と同じ順に押す。帯の秒・時間切れ・持ち出し・返歌・次の作品・一覧の絞り込み・スマホ幅まで | 136項目<!--count:ブラウザ試験--> |
...
| `npm run test:tools` | **本番へ書く道具が、当てる前に断るべきものを断るか。**名前の取り違え・重複・当てる先の食い違いを断ること、途中で落ちたら1本も残らないこと、検査の後片づけがその実行で作った行だけを消し、消し損ねを合格にしないこと、検査用の利用者の見分けかた（3つの形・1か所）と片づけ道具の止まり方、初回利用の進みぐあいを数える SQL が人数を二重に数えないこと、本番スモークが自分の作った匿名ゲストを ID で名指しして消せること（途中で止まっても・2回呼んでも・すでに居なくても安全で、消せなければ不合格になること）を試す | 61項目<!--count:道具の自己試験--> |
