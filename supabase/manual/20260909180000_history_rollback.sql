-- ============================================================================
-- 20260909180000_history_rollback.sql ／ 上の修復を取り消し、修復前の行へ戻す
-- ============================================================================
--
-- 修復後の行（profile_rpc_revoke_anon / 11文）であることを確かめてから、
-- 2026-09-17 に本番から読み取った修復前の行（billing_founding_creator_v0 / 106文）へ戻す。
-- 106文の中身は、そのとき読んだ本番の行をそのまま文字列にしたもの。
-- CLI では戻せない（CLI は手元のファイルからしか行を作れず、106文の下書きは
-- git に残っていない）ので、戻すのはこの SQL だけ。
--
-- 直接実行しない。node scripts/db-history-0909.mjs rollback から流す。
-- ============================================================================

-- 戻す: 履歴の1行を、修復前（本番で 2026-09-17 に読んだ行）へ戻す
...
begin
  ...
  if not exists (select 1 from supabase_migrations.schema_migrations where version = '20260909180000' and name = 'profile_rpc_revoke_anon' and md5(statements::text) = 'e2158bec568e70801cefdfeff057b193' and array_length(statements, 1) = 11) then
    raise exception 'STOP: 対象行が想定の状態（name=profile_rpc_revoke_anon / 11文 / md5 e2158bec568e70801cefdfeff057b193）ではない';
  end if;
  update supabase_migrations.schema_migrations
     set name = 'billing_founding_creator_v0', statements = '{"-- ============================================================================
...
$fn$","comment on function public.app_billing_touch_updated_at() is
  ''課金の表の updated_at を書き換えの都度そろえる【内部用】。誰も直接呼べない。''","revoke all on function public.app_billing_touch_updated_at()
...
create table if not exists public.billing_offers (
  ...
  ''ブラウザから送られた金額は一切信用せず、この行と突き合わせる。''","comment on column public.billing_offers.sales_cap is
  ''同時に有効でいられる人数の上限。返金で1枠戻るので、通し番号の最大値は上限を超えうる。''","comment on column public.billing_offers.is_active is
  ''既定は false。行を足しただけでは売れない。''","create trigger billing_offers_touch_updated_at
  before update on public.billing_offers
  for each row
  execute function public.app_billing_touch_updated_at()","alter table public.billing_offers enable row level security","revoke all on table public.billing_offers from public, anon, authenticated","-- Founding Creator（D187）。3,000円・買い切り・有効30名。
...
create table if not exists public.billing_customers (
  ...
  ''アプリの利用者と Stripe の顧客の対応。退会しても記録を残せるよう profile_id は切れる。''","create trigger billing_customers_touch_updated_at
  before update on public.billing_customers
  for each row
  execute function public.app_billing_touch_updated_at()","alter table public.billing_customers enable row level security","revoke all on table public.billing_customers from public, anon, authenticated","-- ============================================================================
...
create table if not exists public.billing_purchases (
  ...
  ''枠を数えるのは reserved / paid / refund_pending / disputed の4状態。''","comment on column public.billing_purchases.founder_number is
  ''決済の確定順。返金しても消さない（欠番として一覧に残す）。再利用しない。''","comment on column public.billing_purchases.founder_public is
  ''一覧とプロフィールに名前を出すか。既定は非公開。本人だけが変えられる。''","comment on column public.billing_purchases.pro_trial_redeemed_at is
  ''Creator Pro の90日権を受け取った時刻。今回はまだ誰にも入らない。''","create trigger billing_purchases_touch_updated_at
  before update on public.billing_purchases
  for each row
  execute function public.app_billing_touch_updated_at()","-- **同じ人・同じ商品で、生きている購入は1つだけ。**
...
create unique index if not exists billing_purchases_one_live_per_profile
  ...
  where profile_id is not null
    and status in (''reserved'', ''paid'', ''refund_pending'', ''disputed'')","-- 残り枠の勘定に使う
create index if not exists billing_purchases_offer_status_idx
  on public.billing_purchases (offer_id, status)","-- 一覧は番号順に並べる
create index if not exists billing_purchases_founder_number_idx
  on public.billing_purchases (founder_number)
  where founder_number is not null","alter table public.billing_purchases enable row level security","revoke all on table public.billing_purchases from public, anon, authenticated","-- ============================================================================
...
create table if not exists public.billing_entitlements (
  ...
  ''取り消すときは行を消さず revoked_at を入れる。''","alter table public.billing_entitlements enable row level security","revoke all on table public.billing_entitlements from public, anon, authenticated","-- ============================================================================
...
create table if not exists public.billing_webhook_events (
  ...
  ''受け取った Stripe の合図。ID が主キーなので、同じ合図の再送では2度目の insert が通らない。''","create index if not exists billing_webhook_events_unprocessed_idx
  on public.billing_webhook_events (created_at)
  where processed_at is null","alter table public.billing_webhook_events enable row level security","revoke all on table public.billing_webhook_events from public, anon, authenticated","-- ============================================================================
-- 6. 枠の勘定（内部用）
-- ============================================================================
--
...
$fn$","comment on function public.billing_active_slots(bigint) is
  ''いま枠を占めている件数【内部用】。reserved / paid / refund_pending / disputed の4状態。''","revoke all on function public.billing_active_slots(bigint)
  from public, anon, authenticated","-- 売り出し中かどうか。is_active と期間の両方を見る
...
$fn$","comment on function public.billing_offer_is_open(public.billing_offers) is
  ''その商品がいま売り出し中か【内部用】。有効の印と期間の両方を見る。''","revoke all on function public.billing_offer_is_open(public.billing_offers)
  from public, anon, authenticated","-- ============================================================================
...
$fn$","comment on function public.billing_reserve_slot(uuid, text) is
  ''決済ページを作る前に販売枠を1つ押さえる。商品の行で錠を取るので上限を超えない。''
  ''同じ人の生きている購入が既にあれば、新しい行を作らずそれを返す（連打対策）。''","revoke all on function public.billing_reserve_slot(uuid, text)
  from public, anon, authenticated","grant execute on function public.billing_reserve_slot(uuid, text) to service_role","-- ============================================================================
...
$fn$","comment on function public.billing_upsert_customer(uuid, text) is
  ''この人の Stripe 顧客を1つに保つ。既にあれば ID を上書きする。''","revoke all on function public.billing_upsert_customer(uuid, text)
  from public, anon, authenticated","grant execute on function public.billing_upsert_customer(uuid, text) to service_role","-- 既に結び付いている Stripe 顧客を読む（無ければ null）
...
$fn$","comment on function public.billing_get_customer(uuid) is
  ''この人の Stripe 顧客。無ければ null。''","revoke all on function public.billing_get_customer(uuid)
  from public, anon, authenticated","grant execute on function public.billing_get_customer(uuid) to service_role","-- ============================================================================
...
$fn$","comment on function public.billing_attach_checkout(uuid, uuid, text, timestamptz) is
  ''作った決済ページを、予約中の購入へ結び付ける。予約中以外には結び付けない。''","revoke all on function public.billing_attach_checkout(uuid, uuid, text, timestamptz)
  from public, anon, authenticated","grant execute on function public.billing_attach_checkout(uuid, uuid, text, timestamptz)
  to service_role","-- ============================================================================
...
$fn$","comment on function public.billing_complete_checkout(text, uuid, text, text, text, integer, text, text) is
  ''決済の確定。売り方・支払い状態・決済ページ・商品・金額・通貨の6つが合ったときだけ''
  ''Founder 番号を出し、founding_creator と beta_access を付ける。''
  ''同じ合図が2回来ても番号は増えない。''","revoke all on function public.billing_complete_checkout(text, uuid, text, text, text, integer, text, text)
  from public, anon, authenticated","grant execute on function public.billing_complete_checkout(text, uuid, text, text, text, integer, text, text)
  to service_role","-- ============================================================================
...
$fn$","comment on function public.billing_expire_checkout(text) is
  ''決済ページの失効を受けて枠を戻す。予約中の行だけを動かす。''
  ''DB の時計では戻さない。Stripe が失効したと言ったときだけ。''","revoke all on function public.billing_expire_checkout(text)
  from public, anon, authenticated","grant execute on function public.billing_expire_checkout(text) to service_role","-- ============================================================================
...
$fn$","comment on function public.billing_mark_refund_pending(text, text) is
  ''返金の手続きが始まったことを記録する。**権利はまだ取り上げない。**''","revoke all on function public.billing_mark_refund_pending(text, text)
  from public, anon, authenticated","grant execute on function public.billing_mark_refund_pending(text, text) to service_role","-- 返金の結果を反映する。p_refund_status は Stripe の refund.status そのまま
...
$fn$","comment on function public.billing_apply_refund_result(text, text, text) is
  ''返金の結果を反映する。succeeded で権利を取り上げて枠を戻し、''
  ''failed / canceled では元の権利を維持する。番号は再利用しない。''","revoke all on function public.billing_apply_refund_result(text, text, text)
  from public, anon, authenticated","grant execute on function public.billing_apply_refund_result(text, text, text)
  to service_role","-- ============================================================================
...
$fn$","comment on function public.billing_mark_dispute(text, text) is
  ''不審請求の申し立てを反映する。途中は disputed（枠は押さえたまま）、''
  ''lost で reversed にして権利を取り上げ、won / warning_closed / prevented で paid へ戻す。''","revoke all on function public.billing_mark_dispute(text, text)
  from public, anon, authenticated","grant execute on function public.billing_mark_dispute(text, text) to service_role","-- ============================================================================
-- 14. Webhook の重複防止
-- ============================================================================
--
-- 【claim → 処理 → done の3手】
...
begin
  insert into public.billing_webhook_events
    (stripe_event_id, event_type, object_id, livemode, stripe_created_at)
  values
...
$fn$","comment on function public.billing_claim_webhook_event(text, text, text, boolean, timestamptz) is
  ''この合図を初めて受け取ったかを返す。2回目以降は claimed=false。''","revoke all on function public.billing_claim_webhook_event(text, text, text, boolean, timestamptz)
  from public, anon, authenticated","grant execute on function public.billing_claim_webhook_event(text, text, text, boolean, timestamptz)
  to service_role","create or replace function public.billing_finish_webhook_event(p_event_id text)
returns jsonb
language plpgsql
security definer
set search_path = ''''
...
$fn$","comment on function public.billing_finish_webhook_event(text) is
  ''その合図を処理し終えたことを記録する。''","revoke all on function public.billing_finish_webhook_event(text)
  from public, anon, authenticated","grant execute on function public.billing_finish_webhook_event(text) to service_role","-- ============================================================================
...
$fn$","comment on function public.get_founder_offer_status(text) is
  ''Founder の残り枠と、いまの人の購入状態。決済ページの ID も URL も返さない。''","revoke all on function public.get_founder_offer_status(text)
  from public, anon, authenticated","grant execute on function public.get_founder_offer_status(text)
...
$fn$","comment on function public.list_founders(text) is
  ''Founder 一覧。番号順。公開／匿名希望／欠番の3つの見え方だけを返す。''
  ''金額・決済 ID・profile_id は返さない。''","revoke all on function public.list_founders(text)
  from public, anon, authenticated","grant execute on function public.list_founders(text) to anon, authenticated","-- ============================================================================
...
as $fn$
  ...
  limit 1;
$fn$","comment on function public.get_founder_badge(uuid) is
  ''その人のプロフィールに出す Founder 番号。公開設定が入で、権利が生きているときだけ返す。''","revoke all on function public.get_founder_badge(uuid)
  from public, anon, authenticated","grant execute on function public.get_founder_badge(uuid) to anon, authenticated","-- ============================================================================
...
$fn$","comment on function public.set_my_founder_visibility(boolean) is
  ''Founder の名前を一覧とプロフィールに出すかを切り替える。本人の行だけを動かす。''","revoke all on function public.set_my_founder_visibility(boolean)
  from public, anon, authenticated","grant execute on function public.set_my_founder_visibility(boolean) to authenticated","-- ============================================================================
...
$fn$","comment on function public.get_my_entitlements() is
  ''いま生きている自分の権限の鍵の一覧。取り消し済みと期限切れは入らない。''","revoke all on function public.get_my_entitlements()
  from public, anon, authenticated","grant execute on function public.get_my_entitlements() to authenticated"}'::text[]
   where version = '20260909180000' and name = 'profile_rpc_revoke_anon' and md5(statements::text) = 'e2158bec568e70801cefdfeff057b193';
  ...
  if not exists (select 1 from supabase_migrations.schema_migrations where version = '20260909180000' and name = 'billing_founding_creator_v0' and md5(statements::text) = 'e98153b0892a896ab75bb24860515720' and array_length(statements, 1) = 106) then
    raise exception 'STOP: 更新後の行が想定（name=billing_founding_creator_v0 / 106文 / md5 e98153b0892a896ab75bb24860515720）と違う';
  end if;
  select count(*) into v_total from supabase_migrations.schema_migrations;
