...
|---|---|---|---|
| A 既存絵からクイズを作る | **2026-09-08 に確定し、実装した → D172。**この行の中身は決定ログへ移した | UI の正式名称・画像解析・自由入力・origin 別の見せ方（D172 の未確定） | 無し |
...
## 候補A 既存絵からクイズを作る（逆方向モード）→ 2026-09-08 に確定・実装（D172）

**この節は候補ではなくなった。**確定した内容は `docs/decisions.md` の D172、
実装前の調査は `docs/art-first-investigation.md` にある。以下は当時の記録。
...
1. art-first モードの正式名称
   → 2026-09-08 に内部名称だけ確定（`art_first`）。UI の文言は未確定のまま
2. origin 列・enum の具体名
   → 2026-09-08 に確定（`prompts.origin` に `art_first` を足す。新しい列は作らない）
