...
本番へ未適用の migration が 6本ある（全部で 52本<!--count:migration-->）。
出所: **2026-09-09 に本番の履歴表と手元を突き合わせた実測**（`npm run db:status`。
本番に記録があるのは49本、手元は55本、差が6本）。
**画面を公開する前に、これを当てる。**

未適用の6本（当てる順もこのとおり。入れ替えられない）:
...
| 数え方 | 本数 | 意味 |
|---|---|---|
| 本番未適用 | 6本 | 本番のデータベースにまだ入っていない |
| Git 未コミット | 9本 | まだコミットしていないファイル |

この2つは重ならない。管理 v0 と課金 v0 の migration 3本は、
**コミットしていないのに本番へは既に入っている。**

本番未適用の6本は、当てても `npm run db:deploy` では通らない。
6本とも本番の最後（`20260909200000`）より前の時刻なので、
Supabase CLI が「最後より前への挿入」として断る（2026-09-09 実測）。
