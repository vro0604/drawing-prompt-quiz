...
| この作業木に無い | 5本 | 本番には入っているが、この作業木のファイルには無い |

「この作業木に無い5本」は、別の作業線がこの本流とは別に作って本番へ当てたもの。
管理 v0（`20260908130000`）と、プロフィールの権限（`20260909180000`）、
形状アシスト（`20260909200000`）、回答の知らせ（`20260910090000`）、
サブ指令（`20260910140000`）。
...
本番未適用の7本は、当てても `npm run db:deploy` では通らない。
6本は本番の最後（`20260910140000`）より前の時刻で、Supabase CLI が
「最後より前への挿入」として断る（2026-09-09 実測）。
