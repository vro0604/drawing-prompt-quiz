...
## 手順 -1. 先に migration を本番へ当てる（2026-09-10 に書き直し）
...
2026-09-21 時点の本数は 71本<!--count:migration-->。本番には69本が適用済みで、自由額支援と法務表示の2本が未適用。
以下の68本という数字は2026-09-18 の実測記録。
69本目の共有カード（`20260918170000_share_cards`）は、この作業線で足したもの。
68本目の作品一覧（`20260918120000_feed_masonry`。D211）は 2026-09-18T05:29Z に
`npm run db:deploy` で当てた。当てた瞬間に走る文は 表1・索引1・関数3・権限9・RLS1 で、
破壊や更新にあたる文は1本も無い（`npm run audit:migration-ops` の実測）。
当てたあと `npm run db:verify:keychain` が 239項目すべて合格。表は 69 → 70。
...
適用後の db:verify:keychain は236項目すべて期待どおり。規約・ポリシーの有効版は 2026-08-08 → 2026-09-09 に移り（当初 2026-09-08 と書いたのは誤り。D204）、
...
適用後の db:verify:keychain は222項目すべて期待どおり（適用前は、同じ検査の旧版で209項目すべて期待どおり）。
60本目の `20260911120000_cleanup_failed_orphan_prompts`（持ち主のいない時間切れ（failed）と
自動破棄（discarded）のお題を、掃除の対象と残り件数に含める）は 2026-09-11T13:39Z に
`npm run db:deploy` で適用。適用後の db:verify:keychain は209項目すべて期待どおり。
（その1本前の `20260910210000_touch_session_as_owner` は 2026-09-11T07:59Z に
...
および PostgreSQL 17.5 に同じ構造を作って再現したこと）。

当てた7本（当てた順もこのとおり。入れ替えられない）:

```
20260907120000_single_pass_draw_and_slot_redo    （P0）
20260910190000_merge_draft_state_contract        （P0 の直後に置く。下の注を読むこと）
...
時刻の順ではなく、この順で当てる。`20260910190000` だけが時刻の上では最後だが、
**P0 の直後に置く。**理由は、P0 が盤面の読み出し口（`draft_state_json`）を作り直す
ときに、別の作業線が先に本番へ入れた形状アシスト（D191）とサブ指令（D193）の
鍵を落としてしまうため。`20260910190000` はその2つの鍵を戻し、P0 の鍵も
そのまま返す1本にまとめる。
出所: **2026-09-10 の実測**（本番と同じ並びを手元に作り、P0 だけを当てると
`draft_state_json` から `shape_assist_key` と `sub_directive_key` が消えることを確認）。

### 切り替えの4段（A → B → C → D。続けて行う）

**A. 7本を1つのトランザクションで当てる**

```
npm run db:apply:many -- \
  20260907120000_single_pass_draw_and_slot_redo.sql \
  20260910190000_merge_draft_state_contract.sql \
  20260908120000_answer_analysis.sql \
  20260908160000_analysis_drilldown.sql \
  20260909090000_import_capacity.sql \
  20260909120000_consent_gate_and_flavor_compose.sql \
  20260909150000_overrun_inactivity_notify.sql \
  --confirm-project <link しているプロジェクトの識別子>
```

`--confirm-project` は、当てる先を取り違えていないかの確認。link されている先と
一致しなければ、接続する前に断る。先に `--dry-run` を付けて、当てる順と本数を
目で確かめてから実行する（`--dry-run` のときは `--confirm-project` は要らない）。

**7本は全部入るか、1本も入らないかのどちらかになる。**途中で失敗すれば
トランザクションごと戻り、データベースは当てる前のままになる。
出所: **2026-09-10 の実測**（本番と同じ土台に、失敗する SQL を最初・途中・最後の
3か所に差し込んで試し、3回とも 関数144本・表52個 に完全に戻った）。

`npm run db:deploy`（= supabase db push）は使えない。7本のうち6本は本番の最後
（`20260910140000`）より前の時刻で、Supabase CLI が「最後より前への挿入」として
断るため（2026-09-09 実測）。

**B. 当てた7つの版番号を、履歴表へ記録する**

```
npx supabase migration repair 20260907120000 --status applied --linked
npx supabase migration repair 20260910190000 --status applied --linked
npx supabase migration repair 20260908120000 --status applied --linked
npx supabase migration repair 20260908160000 --status applied --linked
npx supabase migration repair 20260909090000 --status applied --linked
npx supabase migration repair 20260909120000 --status applied --linked
npx supabase migration repair 20260909150000 --status applied --linked
```

A の道具は履歴表に触れない。**SQL が入ったことと、履歴に書いたことを、
別々の失敗の境目として扱うため。**A が成功して B が失敗したときは、
SQL を流し直すのではなく B だけをやり直す。A を流し直すと、
制約を足す文が「もうある」で失敗する。

逆に、当てていないものを B だけ実行するのは履歴に嘘を書く行為なので絶対にしない。
...
Vercel の Production へ、統合した作業線を出す。

### A から D までのあいだに起きること

データベースの切り替えそのものは、A の commit の一瞬で終わる。
途中の状態が本番に見えることはない。

**ただし A が終わってから D が終わるまでは、古いアプリと新しいデータベースの
組み合わせになる。**この間、古いアプリの次の操作が動かない。

- カードを「めくる」「これに決める」（新しいデータベースでは1回で引いて決める形に変わる）
- 掃除の2段（`expire_overdue_prompts` と `expire_overdue_drafts` が作り直される）

データは壊れない。壊れるのは操作の途中だけで、画面を出し直せば新しい形で続けられる。

したがって、**A から D までは続けて行う。**利用の少ない時間帯を選ぶ。
出所: アップグレード試験の「順番3: 最後の1本を当てると、めくっても決まらなくなる」
（`npm run test:db:upgrade`）。

### 「本番未適用」と「この作業木に無い」は別のもの

数え方が2つあるので、同じ「未適用」という言葉で呼ばない。

| 数え方 | 本数 | 意味 |
|---|---|---|
| 本番未適用 | 0本（2026-09-17 の実測。当時は7本） | 本番のデータベースにまだ入っていない |
| この作業木に無い | 0本 | 本番にあって、この作業木のファイルに無いもの |

以前はここが5本だった。管理 v0・プロフィールの権限・形状アシスト・回答の知らせ・
サブ指令が、別の作業線にしか無かったため。いまはこの作業木が origin/main を土台に
しているので、本番にあるものは全部ここにもある（2026-09-10 の実測）。

（2026-09-17 追記: 課金 v0 の2本は 20260917090000 / 100000 として本番へ入った。以下はそれ以前の記録。確かめ直しは D204）

**課金 v0 の2本は本番に入っていない。**手元の main にはあるが、
`20260909180000` と `20260909200000` という版番号を別の作業線が先に使ってしまい、
本番の履歴はその別の作業線のほうを「適用済み」として記録している。
出所: **2026-09-10 の実測**（本番に課金の表・関数が0件、`draft_sessions.shape_assist_key`
は存在。2026-09-09 の記録にあった「課金 v0 は適用済み」は誤りだった）。

2026-09-12 に、課金側の2本を未使用の版番号へ振り直した（枝 `billing-renumber`）。
`20260912140000_billing_founding_creator_v0.sql` と
`20260912150000_legal_v2_billing.sql`。SQL の本文は変えていない
（改名の前後で sha256 が同じ）。手元の `main` は動かしていない。

本番の履歴 `20260909180000` の行は、名前も文も課金側のまま残っている（2026-09-17T15:27Z に修復済み。変わったのは履歴の1行だけ。D206）。
これは deploy を壊していない。CLI は版番号だけで突き合わせるため。
出所: **2026-09-12 の実測**（`supabase migration list` は60本すべて一致・警告なし、
`supabase db push --dry-run` は `Remote database is up to date.`）。
将来この課金を本番へ入れるときは、振り直した番号で当て直しを検証する。

**2026-09-17 に課金を main へ取り込み、版番号をもう一度振り直した。**
`20260912140000` → `20260917090000`、`20260912150000` → `20260917100000`。
理由は、9/16 に本番へ `20260914120000`（時計）が入り、本番の最大がこれより後ろへ動いたため。
本番の最大より前の番号は `npm run db:deploy` では当てられない
（CLI が「最後より前への挿入」として断る。上の「手順 -1」の A の注と同じ。`--include-all` は使わない決まり）。
SQL の本文は変えていない（sha256 は `997f9e9b…` / `d9711e75…` のまま）。
取り込み後の実測（2026-09-17、読むだけ）: `supabase migration list` は66行のうち
64本が手元と本番の両方にあり、手元だけにあるのが課金の2本、本番だけにあるものは無し。
`supabase db push --dry-run` が当てようとするのも、この2本だけ。**本番へは当てていない。**
...
**互換化しきれていなかった1件は、2026-09-09 に消えた。**制作の期限のガードが
旧い画面に当たる問題は、期限の超過で投稿を止めるのをやめた（D183）ため
起きなくなった。いま投稿の受け口が止めるのは「48時間の放置で自動破棄された
作りかけ」だけで、これは旧い画面でも新しい画面でも同じように止まる。
当時の記録は `docs/prod-audit-2026-09-06.md` の 2-4 に残してある。
...
- 制作タイマー（延長・超過中の表示・放置による自動破棄。猶予と失敗は 2026-09-09 に撤去）
...
- 未適用 migration 6本（2026-09-09 の実測。`npm run db:status`。上の一覧と同じ6本）
...
- [ ] プッシュ通知を使うなら、`VAPID_*` の4つも入れる（`docs/web-push-setup.md`）。
      入れなくてもサイトは動き、知らせは画面内にだけ出る
