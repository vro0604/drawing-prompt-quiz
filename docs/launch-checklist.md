...
本番へ未適用の migration が 7本ある（全部で 58本<!--count:migration-->）。
出所: **2026-09-10 に本番の履歴表と手元を突き合わせた実測**（`npm run db:status`。
本番の履歴に載っているのは51本、この作業木は58本、その51本すべてが本番と重なる）。
**画面を公開する前に、これを当てる。**

未適用の7本（当てる順もこのとおり。入れ替えられない）:
