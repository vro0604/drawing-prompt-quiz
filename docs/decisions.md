...
## D167. 制作挑戦の時計（旧決定：ドラフト開始時から計測）

**2026-09-14 改定（ユーザー指示）:** 制作時間の起点を「お題の確定時刻」に変更した。
以下は変更前の決定記録。ドラフト中は制作期限も時計の帯もなく、確定時に
`prompts.started_at` と `deadline_at` を設定する。カード選択にかかった時間は
制作時間・実経過に含めない。放置による48時間の自動破棄は引き続き有効。
...
### D189. 一巡で仮のお題を作り、カテゴリごとに1回だけ引き直す（D170 / D171 の画面部分を置き換える）

> **番号について（2026-09-09 に振り直した）。**この節は当初 D172 を名乗っていたが、
> 「既存の絵を持ち込む経路（art_first）」が同じ 2026-09-08 に同じ番号を取っていた。
> art_first の側は既にコミット済みなので動かさず、**まだコミットしていない
> こちらだけを D189（当時の末尾 D188 の次の未使用番号）へ移した。**
> 出所: ユーザー指示（2026-09-09）「未commitのP0側だけを新しいD番号へ変更する」。
> **この節だけ、この文書の中で番号の並び順から外れている。**
> 前後の節（D171 と D174）とのあいだに置いたまま動かしていない。

出所: ユーザー指示（2026-09-08）「現在本番に入っているドローUIは最新仕様ではない」
「UI・状態遷移として古い『確認 / 保持』方式を残してはいけない」。

#### 置き換えたもの（過去の決定と、いまの決定）

| 過去の決定 | いまの決定 |
|---|---|
| D170 めくることと決めることを分ける | 引いた瞬間に仮採用。確認を挟まない |
| D170 枠の中で候補を残す（保持） | 画面から外した。DB の hold_card は残す |
| D170 残したうえで枠の残りを開く | 画面から外した。残りが開くのは引き直したときだけ |
| D171 開示した枠があると全体を引き直せない | 引き直したカテゴリがあると全体を引き直せない（同じ趣旨を広げた） |

**D170 と D171 のうち、上の4行は置き換えた。**それ以外
（候補の配分、モーフ上限2、カラー任意、お題の放棄、他の候補を見る、
カテゴリ表示名の集約、タイマーの一本化、メール確認の元タブ反映）は
そのまま生きている。

#### 新しい流れ

一巡目。カテゴリごとに伏せカードを1枚引く。引いた瞬間に仮採用になり、
確認を挟まずに次のカテゴリへ進む。一巡し終えて初めて、仮のお題が
まとまって見える。**一巡中は「これに決める」「残しておく」「残りを見る」を
1つも出さない。**

カテゴリごとの引き直し。1カテゴリにつき1回だけ。押しても、その場では
まだ捨てない。取り消せないことを伝える画面を1枚はさむ。そこで決めると、
仮採用のカードを永久に捨て、灰色にして大きな × を付け、短い退場の動きを
出し、そのカテゴリの残り候補を開き、そこから1枚選んで確定する。

#### 捨てたカードが戻らないことを、どこで保証しているか

`draft_candidates.is_discarded` に印を付ける。**行は消さない。**
何を捨てたかが見えなくなると、引き直した意味が分からなくなるため。

戻らないことは画面ではなく DB が守る。`pick_card` は印の付いたカードを
`CARD_DISCARDED` で断る。`redo_slot` は2回目を `REDO_ALREADY_USED` で断る。
だから読み込み直しても、戻るボタンで送り直しても、同じ要求が2本同時に
届いても、2枚目は捨てられず、捨てたカードは選択肢に戻らない。

#### 消さなかったもの

`reveal_card` `choose_card` `hold_card` `reveal_slot_pool` と、
`draft_candidates.is_held` `draft_session_slots.pool_revealed_at` は残した。
出所: ユーザー指示「既存 migration / column を無理に消す必要はない」
「いきなり drop せず、『使わなくする』だけで済むならその方を優先」。
画面からは呼ばれない。既に動いているドラフトが壊れないためだけに残っている。

#### 判断が要る点（提案。まだ決まっていない）

ドラフト全体の引き直し（`reroll_draft`）を残すかどうか。今回の指示に
書かれていないので、**残したうえで、カテゴリを1つでも引き直したら
断る**ようにした。理由は、全体の引き直しが候補をまるごと作り直すので、
捨てたカードを「無かったこと」にしてしまうため。
廃止するか残すかは未決。

---

### D174. 回答は1セクションずつ。絵を見たまま、長押しで確定する

（番号について。D172 は一時2つあった。持ち込み（art_first）と一巡ドローが
別々の作業で同じ番号を取ったためで、**2026-09-09 に一巡ドローの側を D189 へ
振り直して解消した。**ここは当時の次の空き番号を使っており、173 は空けたままにする。）

出所: ユーザー指示（2026-09-08）「P1：回答UI全面変更」。

#### 何が変わったか

これまでは、問が縦に全部並んでいた。作品の絵はページのいちばん上に1枚あり、
3問目を読むころには画面から消えている。**絵を当てる遊びなのに、
絵を見ずに答える形になっていた。**

いまは、絵が上に貼り付いたまま動かない。下半分だけが1セクションずつ
入れ替わる。確定はカードの長押しで、押している間、円のゲージが左回りに満ちる。
...
- 長押しで確定するまで … 1.5 秒
- 100%未満で離したときに 0 へ戻りきるまで … 2.3 秒
- 確定してから次のセクションへ移るまで … 0.52 秒

100%未満で離しても、**その場では 0 に戻らない。**ゆっくり減る途中で
押し直すと、残っている量から続く。0.8 秒押して離し、0.4 秒後に押し直したら、
残っているぶんの続きから始まる。「押し直したら最初から」にはしない。

見た目の充填は序盤が速く終盤が遅い（ease-out）。**この曲線は確定の時刻を
1ミリ秒も動かさない。**曲線と判定を別の場所に置いてある
（`src/features/quiz/hold.ts` の `gaugeFraction` と `HOLD_MS`）。

#### 何が何を呼び、値がどこからどこへ渡るか

出題はこれまでどおりサーバーから降りてくる（`get_work_quiz`）。正解は入っていない。
選んだ語は回答の画面の中だけに溜まり、DB へは1文字も行かない。
最終確認で「回答する」を押すと、溜めた語が hidden の入力欄になって
`submitAnswerAction` へ渡り、そこから `submit_answer` へ渡る。
採点はこれまでどおり DB の中だけ。

**書き込みは最後の1回だけ。**途中のセクションでは何も保存しない。
1作品1回という決まりは増やしも減らしもしていない。

#### 読み込み直したときに残るもの、残らないもの

残るのは、同じタブの中だけ。ブラウザの一時的な預かり（sessionStorage）に
置いてあるので、読み込み直せば続きから再開できる。

**別の端末・別のブラウザ・タブを閉じたあとには何も残らない。**
途中の回答をサーバーへ置く仕組みは、いまの DB に無い。作るなら
「1作品1回」を守ったまま未完成の回答を持てる表が要る。
今回は作っていない（P1 の範囲外）。

#### 語が何回出されたかを、あとから数えられること

出された選択肢は `quiz_choices` に残っている。お題を確定したときに
1回だけ抽選して書き込み、そのあとは誰が見ても同じものが出る。
**人ごとに並べ替えたり選び直したりしていない。**
だから回答1件ごとに「その人に何が出ていたか」を復元できる。
別の控えを取る必要は無い。実測で確かめた（`test/db/run.mjs` の U 群3件）。

#### まだ決めていない

複勝（2語で答える方式）の操作方法。「2語をそれぞれ長押しするのか」
「2語を選んでから、まとめて1回長押しするのか」は決まっていない。
いまは後者で動かしているが、**暫定（私が設定）。**
既存の画面が「全部選んでから1回送る」形だったので、それに近いほうを置いた。
部品は、押す対象を差し替えるだけでどちらにもできる形にしてある。

呼び名も決まっていない。ユーザーの言葉は「複勝」、これまでの画面の言葉は
「2択当て」。**回答中の切り替えボタンだけ「複勝」にし、結果の画面は
「2択当て」のままにしてある。**どちらに揃えるかは未決。

---

### D175. 結果の主役を、当てられた割合から「どう見えたか」へ移す

出所: ユーザー指示（2026-09-08）「P2で実装するもの」。

#### 何が変わったか

作者が結果を開いたとき、いちばん上に出るものが変わった。

これまでは「67%」のような、当てられた割合の大きな数字だった。
その数は当たったかどうかしか言わない。**自分の絵がどう見えたかは
何も分からない。**

いまは、項目ごとに「どの語が選ばれたか」が並ぶ。その下に、
どの項目で正解を含んだ人がどれだけ重なっているかの図が出る。
当てられた割合は消していない。小さくして、その下に残した。

回答した人の画面には「他の人は、どう見たか」が付いた。
項目ごとにみんなが選んだ語（自分の答えに印が付く）と、
自分と同じ答えをした人がどれだけいるかが出る。

#### 語の並べ方

語ごとに3つの数を持つ。ビタ当てで選ばれた回数、複勝の2語のどちらかとして
選ばれた回数、そして選択肢として目の前に出た回数。

順位は「出た回数あたりの点」で決める。点はビタ当てが 1.0、複勝が 0.5。
ビタ18・複勝26 なら点は 31 で、50回出ていれば 0.62。

選ばれた回数だけで並べない理由。語は目の前に出たときにしか選ばれない。
10回出て5回選ばれた語と、2回しか出ていない語を、選ばれた数だけで
並べると後者が沈む。
...
回答者ごとに、問の出題順で 0 と 1 を並べる。5問なら "11010" のような5文字。
1 は「その問で正解を含んでいた」で、ビタ当てで当てた場合と、
複勝の2語のどちらかが正解だった場合の両方が入る。

同じ並びの人数を数える。5問なら 2^5 = 32 通りで、
**誰もそう答えなかった並びも 0 人として出す。**全部外した "00000" も出す。
「誰もいない」と「そういう答え方が無い」は違う。

5つの円のベン図はそのまま描いていない。区画が細く潰れて人数を書けないため。
代わりに区画を1マスずつ並べ、マスの中の点でどの項目を含む区画かを示す。
...
- ビタ当て A と 複勝 A+B … 一致しない

ビタ当てだから加点、複勝だから減点、のような重みは付けない。
「k問以上一致した人」を上から積み上げて出す。母数は自分を除いた回答者で、
**1問も一致しなかった人も母数に入れる。**

#### 完全ビタ

全問をビタ当てで、しかも全問当てた人。複勝で正解を含んでいても入らない。
作者には人数を、本人には特別な表示を出す。

そこに添える「組み合わせ上の希少度」は、各問の選択肢の数から出す。
4択ならビタ当て4通り＋複勝6通りで10通り、5問なら 10^5 = 100,000。
**100,000 という数はどこにも書いていない。**問数も選択肢の数も変わりうる。

これは組み合わせが何通りあるかであって、**当たる確率ではない。**
語の選ばれやすさは均等ではないので、確率とは呼ばない。

#### 伝達率という名前について

今回は定義していない。上のどの数も「伝達率」と呼んでいない。
既にその名前が付いている場所（項目別の伝達率、プロフィールの伝わりやすさ、
作品ページの説明文）は、名前も式も1文字も変えていない。

#### 何が何を呼び、値がどこからどこへ渡るか

新しい migration が読み取り専用の関数を3本足した。表も列も足していない。

`answer_word_stats` が問ごとの語の数を作る。これは内部用で、
誰にも実行権を与えていない。正解の印を含むため。

その外側に2本ある。`get_work_answer_analysis` は作者だけ、
`get_my_answer_analysis` は答え終わった本人だけが呼べる。
どちらも中で `answer_word_stats` を呼び、それに並びの分布や似かたを足して返す。
条件を満たさない相手には null を返す。未サインインは、そもそも実行権が無い。

**割り算はこの3本のどれもしていない。**返すのは数えた素の数だけ。
出た回数で割る式、順位の付け方、積み上げ、希少度の掛け合わせは
`src/features/quiz/results.ts` にある。画面はそこを通してから描く。
式を1か所に置いたのは、DB もブラウザも立てずに試せるようにするため。

#### まだやっていない

区画を押して深掘りすること、誤って選ばれた語の多段の絞り込み、
5人未満のときに隠すこと、除外して数え直すこと。どれも P3 の範囲。
区画は1マスを部品として切り出してあるので、押せるようにするのは
その部品を差し替えるだけで済む。
...
> **この時点では D172 が2回使われていた。**「既存の絵を持ち込む経路（art_first）」と
> 「一巡で仮のお題を作り、カテゴリごとに1回だけ引き直す」が同じ番号を持っていた。
> 別々の作業線が同じ時間帯に番号を取ったため。
> 出所: ユーザー指示（2026-09-09）「D172の既存重複問題は今回勝手に全面整理しない。
> 重複の存在だけ記録する」。
>
> **その後、2026-09-09 の統合監査の承認を受けて解消した。**コミット済みの
> art_first を D172 のまま残し、まだコミットしていない一巡ドローを D189 へ移した。
> 出所: ユーザー指示（2026-09-09）「未commitのP0側だけを新しいD番号へ変更する」。
...
### D199. 形状アシスト。お題ではなく「どういう形として描くか」の出発点（仕様確定・実装延期）

> **番号について（2026-09-12 に振り直した）。**この節は当初 D189 を名乗っていたが、
> 「一巡で仮のお題を作る」決定が 2026-09-09 に同じ D189 へ移ってきていて、
> **この文書の中で D189 が2つあった。**実装の側（D191）が「仕様そのものは D189」と
> 指していた先はこちらなので、参照の少ないこちらを、どの作業線でも未使用の
> D199 へ移した（この文書の末尾は D193、着地待ちの作業線の末尾は D198）。
> 出所: **2026-09-12 の実測**（`### D` で始まる見出し183件を数え、
> 重複していた番号は D189 の1件だけ、欠番は 166 / 167 / 173 / 180〜187）。
> **この節も、D189（一巡ドロー）と同じく番号の並び順から外れている。**
> 置き場所は動かしていない。
...
仕様そのものは D199（2026-09-12 まで D189 を名乗っていた節）で固めてある。
この決定は、**それをどう作ったか**を残す。
...
### D178. 作者が結果を掘り下げる。少人数は DB が返さない。分析から外せる

出所: ユーザー指示（2026-09-08）「P3確定事項」。

#### 何ができるようになったか

作者が、正解を含んだ人の重なりの図の1マスを押せる。押すと、そのマスに
入っている人たちだけに絞られ、その人たちが「正解を含まなかった項目」で
何を選んだかが並ぶ。並んだ語を押すと、その語を選んだ人だけにさらに絞られる。
段数の上限は決めていない。

たとえば「A・B・C は当てたが D と E は外した」という区画を押し、
D で「寄生」を選んだ人に絞り、その人たちが E で何を選んだかを見る、
という掘り方ができる。

#### 押せる部品にした（リンクにしていない）

条件は URL に載せている。戻る・進む・読み込み直しがそのまま効くため。
ただし押す部分はリンクではなく、GET のフォームの中の本物のボタン。
仕様が求めているのは押せる部品であって、リンクは押せる部品ではない。
JavaScript は使っていない。

URL を手で書き換えても構わない。返す・返さないを決めるのは DB 側で、
作品の持ち主でなければ何も返らない。

#### 少人数のときに何も返さない

条件で絞った集団が5人未満になったら、そこで止まる。
**人数も返さない。**「4人です」と出すだけでも、条件を少しずつ変えれば
誰か1人を言い当てられる。

画面で隠すのでは足りない。隠しても通信の中身には乗る。
`get_work_drilldown` が、5人未満のときは人数も語の内訳も返さない。
**作者本人にも返さない。**5という数は `analysis_min_subgroup()` にだけ書いてあり、
画面も試験もそこから読む。

この制限がかかるのは掘り下げだけ。全体の語の分布と重なりの図は、
回答が1件でも出す（P2 のまま）。

#### 分析から外す

作者が、自分の分析に使わない回答を選べる。まとめて選べて、いつでも戻せる。

外しても回答そのものは消えない。答えた本人の結果も、公開の集計も変わらない。
外れるのは、作者が自分の作品ページで見る集計の母数だけ。

そのために `answers` に印を足さず、別の表（`analysis_exclusions`）に置いた。
`answers` の列にすると「回答が無効になった」ように見えるうえ、
その表には集計のトリガが付いているので、外す・戻すのたびに
集計表へ触れてしまう恐れがある。

#### 誰が答えたかを出さない

外す相手を選ぶ一覧が返すのは、作品の中だけの通し番号、答えた日時、
いくつの項目で正解を含んだか、完全ビタか、いま外しているか、だけ。
利用者の ID も表示名も返さない。

当て方の並び（"11010" のような）も1件ずつは返さない。
返すと、この一覧から5人未満の集団の中身を組み立て直せてしまう。
**少人数を守る仕掛けを、別の入口から迂回できる。**
「いくつ含んだか」だけなら、重なりの図が段ごとに出している数と同じ粒度なので、
ここから新しく分かることは無い。

#### 母集団を1か所で決めた

`eligible_answers(work_id)` が「作者の分析に使う回答」を返す。
いまの条件は「その作品への回答のうち、作者が外していないもの」。

作者向けの集計も、掘り下げも、この関数だけを見る。
持ち込み枠の仕組みが入って「取り込み済みのものだけ」が加わるときは、
この関数の中に条件を1つ足せば済み、呼ぶ側は1行も変わらない。

#### 何が何を呼び、値がどこからどこへ渡るか

画面の押しボタンは GET のフォームで、URL に `pattern`（区画）と
`f=<問のID>:<語のID>`（絞り込み）を積む。作品ページはそれを読み、
`get_work_drilldown` へ渡す。渡すのは問と語のIDの組だけで、
SQL の断片は渡さない。関数は、その問がこの作品のものか、
その語がその問の選択肢か、同じ項目に条件が重なっていないかを確かめてから絞る。

絞ったあとの回答IDの集合を `answer_word_stats` へ渡す。この関数は
P2 で作ったものと同じ式のまま、数える相手だけを引数で受け取る形にした。
作者向けは外していない回答、回答者向けは全部、掘り下げは絞った集団を渡す。
**式は1つ、数える相手は3通り。**

#### 回答者側との境界

作者が外した回答は、答えた人の画面には影響しない。
`get_my_answer_analysis` は、外された回答も母数に入れたまま数える。
外すのは作者の見方であって、他人の結果を書き換える権限ではない。

#### まだやっていない

7問以上の作品では、重なりの図が「実際に出た並びだけの一覧」に落ちる（P2 のまま）。
掘り下げの仕掛けはそこでも同じように動くが、見た目は作り直していない。
持ち込み枠・購入・通知・規約は P4 以降。

---

### D179. 作品ごとに回答を取り込む枠。無料の集計と高度な分析で母数を分ける

出所: ユーザー指示（2026-09-09）「P4確定事項」。

#### 何ができるようになったか

作品ごとに「高度な分析へ回せる回答の数」を持てるようになった。

回答は今までどおり全部そのまま保存され、無料で見える集計にも全部が入る。
枠が要るのは、重なりの図のマスを押して「その人たちが何を選んだか」まで
掘り下げるほうだけ。取り込んだ回答が1件も無いあいだ、マスは押せない。

作者は、残り枠を使って古い回答から取り込める。自動で取り込む設定にもできる。
枠を使い切ると自動は切れる。枠を足しても、勝手には入り直さない。

#### お金の受け取りは作っていない

決済業者も金額もパックの件数も決まっていないので、購入の経路は1つも無い。
枠を増やせるのは運営の鍵（service_role）を持っている側だけで、その入口は
画面のどこにも無い。作品の画面には、将来そこに導線が入ることだけ書いてある。

将来、支払いが済んだ知らせを受け取る場所から `grant_import_capacity` を
呼べばつながる。関数は「作品と数と出どころ」を受け取るだけで、
パックの件数を知らない。50でも100でも250でも、決まった数を渡せばよい。

#### 母数を2つに分けた

これまで `eligible_answers` が1つあり、「作者が分析から外していない回答」を
意味していた。P4 からは2つになる。

- `analysis_all_answers` … 無料の集計が数える相手。その作品への回答すべて
- `analysis_advanced_answers` … 高度な分析が数える相手。取り込み済み かつ 外していない

名前を2つに割ったのは、1つの名前のまま意味を変えると、呼んでいる側を
読んだときにどちらの意味か分からなくなるため。

この分け方で、作者が回答を分析から外したときの効きかたが変わった。
外しても無料の集計（語の分布・重なりの図・完全ビタの人数）は減らない。
減るのは掘り下げの対象だけ。

#### 台帳を持つ

枠は数を足すだけにせず、足した記録を1行ずつ残す。いつ・いくつ・どこから来たか。
残量は「台帳の合計 − 取り込んだ件数」で、どちらも表を数えれば出る。
**残量そのものはどこにも保存していない。**保存すると台帳と食い違ったときに
どちらが正しいか決められなくなる。

台帳と取り込みの記録は、作品の行を消せなくする（`on delete restrict`）。
いま作品の行を物理的に消す経路はどこにも無い（実測。migration 全体を検索して
`delete from public.works` が1つも無いことを確かめた）。将来足されたときに、
この台帳が止める側に立つ。支払いの証跡を作品の削除で失わないため。

#### 取り込む順番

答えた時刻の古い順。同じ時刻の回答があるので、そのときは回答の id の小さい順。
この2つで必ず一意に決まる。決めておかないと、同じ状況で走らせても
取り込まれる相手が変わる。

#### 同時に来ても枠を使いすぎない

作品ごとに状態の行を1つ持っていて、枠を消費するときは必ずその行を
`for update` で押さえてから数える。同じ作品へ回答が同時に2件届いても、
片方が待つので、残り1枠を2件で使ってしまうことがない。
取り込みの記録は回答のIDが主キーなので、二重に取り込むこともできない。

#### 残りが少なくなったときの知らせ

2割・5%・0の3つ。同じ目盛りで何度も送らないが、枠を足したらまた送れる。

そのために「枠の世代」を持つ。枠を足すたびに世代が1つ進み、そのときの残量が
割合の分母になる。知らせの記録は世代と目盛りの組で1件だけ入る。

分母を「枠を足した直後の残量」にしたのは、その時点で使える枠の量だからで、
台帳の合計にすると、使い切ったあとに足しても分母だけが増え続ける。
（例が示しているのは1回目の付与だけなので、2回目以降の分母は
この読み方で決めた。2026-09-09 の判断。）

知らせはいまのところ作品の画面にだけ出る。**メールでは送っていない。**
アプリからメールを出す仕組みが1つも無いため（実測。src 配下にメール送信の
経路が無く、設定しているのは Supabase Auth 自身の送信だけ）。
記録の表にはメールを送れた時刻の欄を空けてあり、いまは常に空。
送る仕組みができたら、そこへ時刻が入る。取り込みと配送は分けてあるので、
配送に失敗しても取り込みは巻き戻らない。

#### 消しても残る

作品を下書きに戻しても、削除しても、枠と台帳と取り込みの記録は残る。
作者はその作品の画面を開けば確かめられる。

複製や再投稿の経路はまだ無いが、作られたときは新しい作品として別の id を持つ。
枠も台帳も取り込みも作品の id で引いているので、何もしなければ引き継がれない。

#### 既に動いている作品への影響

表を足すだけで、既存の行は1つも書き換えていない。
いまある作品の枠は0、いまある回答は未取り込み、自動取り込みは切から始まる。
知らないうちに枠が減ることも、無料で見えていたものが見えなくなることもない。

#### 決めていないもの

自動取り込みの初期値は切にした。**これは安全側に倒した暫定で、恒久仕様ではない。**
取り込みは取り消せないので、既定で入れると、作者が知らないうちに枠が減る。
変えるときは `work_import_state.auto_import` の既定値1か所。

「直近で何件増えたか」の期間（24時間なのか7日なのか）は決まっていないので、
その数は出していない。未取り込みの回答について出しているのは、件数と、
いちばん古いもの・新しいものの時刻まで。

---

### D180 規約への同意を登録のときへ移し、止める時点をセッションの開始で決める（P5）

2026-09-09。出所: ユーザー指示「利用規約同意は作品投稿時ではなく、アカウント登録時に必須。
投稿画面には規約checkboxを出さない」。

#### これまでどこで同意を求めていたか

投稿フォームの中。未同意のときだけ同意欄が出て、投稿と一緒に記録していた。
守りは `app_guard_works` が持っていて、未同意の INSERT を断っていた（この守りは残す）。

この置き方には2つの問題があった。規約を読む場面が**作品を出す一歩手前**にあって
読む余裕が無いこと。そして**投稿しない人には一度も出ない**こと。

#### 止める時点をどう決めたか

「未同意なら止める」だけでは、この migration を当てた瞬間に、いま作業中の人が
全員止まる。逆に「投稿のときだけ止める」に戻すと、元の場所へ戻ることになる。

分かれ目にしたのは**そのセッションがいつ始まったか**。

ここで iat（券の発行時刻）を使うと壊れる。アクセス券は1時間ごとに作り直されるので、
ログインしっぱなしの人でも1時間後には「たったいま始まった」ように見え、
**作業の途中で同意画面へ飛ばすことになる。**
使うのは amr（どうやって本人だと確かめたか）に入っている時刻で、これは
券を作り直しても変わらない。amr が無いときだけ iat に落とす
（そのときは1時間ぶんの粗さが残る）。この時刻と、

- 関門を置いた時刻（`consent_gate.installed_at`）
- 未同意の版を掲示した時刻（`published_at`）

の遅いほうを比べて、**セッションのほうが後なら止める。**

| 誰か | iat | 止まるか |
|---|---|---|
| 登録したばかりの人 | いま | 止まる |
| いま作業中の既存利用者 | 関門より前 | 止まらない |
| その人が次にログインしたとき | 関門より後 | 止まる |
| 規約を改定した瞬間に作業中だった人 | 改定より前 | 止まらない |
| 改定後にログインした人 | 改定より後 | 止まる |

これで「当てた瞬間に全員が使えなくなる」ことも「作業の途中で画面が同意画面に
変わる」ことも起きない。判定は `consent_status()` 1本が持ち、画面は
`gate_required` を見るだけで、版の比較も時刻の比較もしない。

**iat が読めないときは止めない。**止める根拠が無いのに止めると、作業中の人を
巻き込む。投稿そのものは `app_guard_works` が別に止めている。

#### 新規登録では、その場で記録できないことがある

ゲストからの昇格は、その場で記録できる（ID が既にあるため）。
まったくの新規は、メールの確認が終わるまで ID が確定しないので**その場では
記録できない。**確認のあと最初に開いたときに /consent が出て、そこで記録する。
どちらの道でも、同意していない人が通常の画面へ進むことはない。

#### 止めない場所

トップ・ランキング・公開プロフィールの3つは止めていない。
この3つは**いま認証を1行も読んでいない**（実測）。止めるには利用者ごとの
生成に変える必要があり、共有できる画面が全部個別生成に変わる。
見えるものは未サインインの訪問者と同じで、そこで提供されるサービスは無い。
**これは判断であって、要求からの逸脱として報告した。**

---

### D181 語彙に分類と助詞を足し、作者が文の区切りを決められるようにする（P5）

2026-09-09。出所: ユーザー指示「カテゴリから語を探して選ぶ。カテゴリは navigation /
discovery のためだけ。カテゴリごとの選択quotaは設けない」「文章は最大3文」
「選択した語彙は最後に順序変更可能。dragだけに依存しない」。

#### これまで作れたもの

語を12個並べたもの1文だけ。語は品詞ごとに一列に並び、探す道が無かった。
助詞が無いので「影 消える」の形から出られず、文の区切りも無かった。

#### 分類は探すためだけ

9つ置いた（もの・場所・現象・動き・ようす・関係・助詞・つなぎ・名）。
**分類ごとに選べる数の枠は作っていない。**同じ分類から5語、他が0語でもよい。
お題の語彙（`draw_categories`）は分類ごとに枠を持っているが、あれとは別のもの。

#### 助詞は語彙の一員にした

別の入力欄を作らなかったのは、**正解を漏らさないための関門を1本のままにする**ため。
入力の道が2本あると、片方だけ関門を通ることになる。

ただし助詞は、関門のうち自動の判定（表記・部分一致・読み・漢字・同義グループ）を
通さない。正解が「のこぎり」のとき助詞の「の」が文字として含まれてしまうが、
**助詞が正解を言い換えることはありえない**ので、これは誤って弾いているだけ。
人が1件ずつ登録した禁止（manual）は助詞にも効く。

#### 句読点は保存しない

文の終わりは語の属性（`break_after`）として持つ。「。」は表示のときに付ける。
**作者が打つ記号ではなく、区切りの結果。**読点は入れていない。

#### 2つの上限を分けた

- 語数 24 … 技術上の上限。DOM と DB を守るための数
- 文数 3 … 文章としての決まり（仕様）

**語数のプロダクト上の上限は未決なので、技術上の上限と同じ値にしてある。
暫定（私が設定）。**もとの12から広げたのは、3文にすると1文4語になるため。
数は `flavor_limits()` 1本が持ち、画面にも試験にも書き写していない。

#### 「名」（固有名詞）は0語で置いた

分類は作ったが、語は入れていない。**安全だと1語ずつ確かめられた固有名詞が
まだ無いため。**固有名詞は正解タグとの関係が語ごとに違う。
構造としては受け入れられるので、確かめた語ができたときに insert 1本で足せる。

#### 回答前に見せるかは変えていない

既存の方針（回答前は任意で開けるヒント、既定は畳んである）のまま。
足した88語も既存90語と同じ関門を通る。実測で、4語のお題でも使える語は
178語中いちばん少ない場合で173語（`npm run check:vocab`）。

---

### D182 制作中の時計を2つの形に分け、しきい値を1か所へ集めた（P5）

2026-09-09。出所: ユーザー指示「制作中ページでは画面最上部・横幅いっぱい・不透明・
大きな中央帯・fixed」「他ページでは細いfixed top banner」「thresholdはまだ未決。
hardcodeをUI複数箇所へ散らさない」。

#### 時計の出どころは1つのまま

`get_active_challenge` → `/api/challenge` → 画面いちばん上の帯。
**この経路は増やしていない。**変えたのは大きさと出す項目だけ。
どちらの形にするかは、いま開いているページが「いまの挑戦へ戻る」の行き先と
同じかどうかで決める。

#### sticky から fixed へ

fixed は場所を占めないので、本文の頭が帯の裏へ潜る。
**帯の高さを実際に測って、同じ高さの空きを下に置く**（ResizeObserver）。
決め打ちにすると、画面の幅・文字の大きさ・出ている行数でずれる。

帯は不透明。段階の色（`danger-tint/10` など）は薄く敷くための半透明の色なので、
**外側に不透明な地を1枚敷き、その上に段階の色を重ねている。**

#### しきい値は暫定（私が設定）

`src/features/challenge/warning.ts` の `THRESHOLDS` 1か所。
**ユーザーの承認を受けていない。恒久仕様ではない。**
...
**その段階へ入るのは、割合と秒の両方が当てはまったときだけ。**
割合だけだと24時間枠が残り72分で点滅を始め、秒だけだと5分枠が最初から
警告のままになる。結果として、短い枠では割合が、長い枠では秒が切り替え点を決める。
...
同じことを色（赤）と「まもなく期限」の文言でも伝えている。
だから動きを減らす設定では点滅だけを止める。**時計そのものは止めない。**
読み上げは段階が変わった瞬間だけ1文を出す。秒は読ませない。

#### 時間切れの動きは変えていない

自動投稿も自動削除も強制失敗も足していない。表示だけを作り直した。

---

### D183 予定終了時刻を過ぎても失敗にしない。消えるのは放置だけ（P5 追加）

決定日 2026-09-09。ユーザー確定。

#### 何が問題だったか

`deadline_at` は名前のとおり「予定終了時刻」のつもりで作ったが、
実際には**作りかけを壊す期限**として働いていた。

    予定終了時刻を過ぎる → 猶予 0.5T が始まる → 猶予も過ぎる
      → status = 'failed' → 投稿できない・延長もできない

利用者からの指摘は2つ。

- 「deadlineは予定終了時刻であって、draft destruction deadlineではない」
- 「今のところ延長できた試しがない」

延長できない理由は実測で確かめた（2026-09-09、PGlite、30分枠）。

| 時点 | can_renew | 結果 |
|---|---|---|
| 残り30分（開始直後） | 0 | RENEW_TOO_EARLY |
| 残り10分 | 0 | RENEW_TOO_EARLY |
| 残り7分 | 1 | 成功 |
| 30秒超過 | 1 | 成功 |
| 10分超過 | 1 | 成功 |
| 20分超過 | 0 | RENEW_TOO_LATE（以後どうやっても押せない） |

押せる窓が「残り 0.25T 〜 超過 0.5T」しかない。30分枠なら 22分30秒〜45分の
23分間だけで、それ以外はボタンが出ない。
...
超過は失敗ではない。**時計も消さず、「失敗」表示にも切り替えず、
超過した量を出す。**「制作時間を 08:32 超過しています」。

#### 撤去したもの

- `time_policy` の `grace_ratio` と `renew_threshold_ratio`（行ごと削除）
- `expire_overdue_prompts` / `expire_overdue_drafts`（関数ごと削除）
- 時計の返り値から `grace_ends_at` / `grace_left_seconds` /
  `renew_opens_at` / `is_expired`
- `assert_draft_not_expired` と `works_guard_prompt_deadline` の中身
  （名前と引数は据え置き。呼び出し側6か所を書き換えないため）

**値ごと消した。**残すと、画面のどこかが古い意味で読み続ける。

既にある `status = 'failed'` の行は書き換えていない。過去の記録として残す。

#### 延長の式（ユーザー確定）

    新しい予定終了時刻 = max(いまの予定終了時刻, 現在時刻) + 0.75T

期限内に押せば、残っている時間の上に積まれる（早く押しても損をしない）。
超過中に押せば、いまから 0.75T（過ぎたぶんは戻らない）。
**押せる時刻の窓は無い。**始まった直後でも、6時間超過していても押せる。

押したあとは、延ばした量と新しい終了予定を返す。画面はそれを文にする
（「22分30秒 延長しました。新しい終了予定は 17:04 です。」）。
**失敗したときは黙って何も起こさない状態にせず、DB が作った理由を出す。**

---

### D184 作りかけが消える条件は「最終有効操作からの経過」だけ（P5 追加）

決定日 2026-09-09。ユーザー確定（24時間・48時間とも）。

#### 2つの時計は別物

| | 起点 | 過ぎると |
|---|---|---|
| 予定終了時刻 | 挑戦の開始 ＋ 選んだ制作時間 | 超過中の表示になる。それだけ |
| 放置の予告 | 最終有効操作 ＋ 24時間 | 「あと1日で破棄されます」を1回出す |
| 自動破棄 | 最終有効操作 ＋ 48時間 | 作りかけを破棄する |

**予定終了時刻を48時間の条件に使わない。**何日超過していても、
操作を続けているあいだは消えない。逆に、超過していなくても
48時間触らなければ消える。

日数は `draft_lifecycle_policy` 1表だけに書く。画面にも関数にも数を書かない。

#### 何を「有効操作」と数えるか

**行が実際に書き換わったとき。**トリガーで拾う。

- ドラフトの行（めくる・引き直す・選ぶ・保持する・確定する）
- 候補の行（`draft_candidates`。めくる・選ぶで書き換わる）
- お題の行（要素の持ち出しなど）
- 延長（上のどれかに含まれる）

**ページを開いただけでは数えない。**読み取りの関数（`get_current_draft` /
`get_active_challenge` / `prompt_timer_json`）はすべて `stable` で、
1バイトも書かない。

入口ごとに1行ずつ書き足す形にしなかったのは、**将来1つ増えたときに
書き忘れると、その操作だけ延命されない**（あるいは逆に、読むだけの関数へ
誤って書いてしまい、開くだけで永久に延命される）から。

#### 当てた瞬間に一斉破棄しない

放置の起点は「最終有効操作」と「規則を入れた時刻」の**遅いほう**。
`draft_lifecycle_policy` の `updated_at` がその時刻を持つ。

これが無いと、migration を当てた瞬間に48時間以上動いていない作りかけが
一斉に消え、**予告を一度も受け取らない人が出る。**
規則の値を変えたときも同じように猶予が入る。

#### 破棄したあとに必ず伝える

破棄の事実は `notification_events` に1行残す。
利用者・作りかけの参照・理由（inactivity）・破棄した時刻・確認した時刻。

**この行は作りかけの行に外部キーを張っていない。**掃除が実体を消したあとでも、
「制作途中のお題を自動的に破棄しました」と伝えられる。
「作りかけが無いから何も表示しない」にはしない。

確認するまで、サイトを開くたびに前へ出る。確認したら履歴に残るだけになる。

#### 掃除の頻度（未解決として報告）

`vercel.json` の Cron は1日1回（`17 3 * * *`）。
したがって24時間の予告も48時間の破棄も、**最大で約24時間遅れて届く。**

破棄の判定そのものは DB 側で正確に48時間として持っている
（`inactivity_marks`）。掃除が遅れても「48時間より早く消える」ことはなく、
時計・投稿・延長のすべてが、掃除を待たずにその時点で正しく振る舞う。

より正確に届けるには Cron を1時間ごと（`17 * * * *`）へ変える必要がある。
**2026-09-10 に検討して、変えないことを決めた。**

理由は料金プランの制限。Vercel の無料プラン（Hobby）は
**Cron を1日1回までしか許さず、それより短い間隔を書くと deploy が失敗する。**

> Hobby accounts are limited to daily cron jobs.
> This cron expression would run more than once per day.
>
> 出所: Vercel 公式文書「Usage & Pricing for Cron Jobs」
> https://vercel.com/docs/cron-jobs/usage-and-pricing （2026-09-10 に参照）

| プラン | 最短の間隔 | 発火の正確さ |
|---|---|---|
| Hobby（いま） | 1日1回 | 指定の時刻から1時間以内（±59分） |
| Pro | 1分に1回 | 分単位 |

出所: ユーザー判断（2026-09-10）「日次のまま据え置き」。
Pro（$20/月）へ上げれば `17 * * * *` の1行に変えるだけで毎時になる。

**毎時にしても壊れないことは先に実測してある**（2026-09-10）。
掃除の3本を1日ぶん（24回）続けて回して、次を確かめた。

- 予定超過の知らせ … 24回まわして1件（`dedupe_key` の一意制約が効く）
- 放置の予告 … 24回まわして1件
- 48時間の破棄 … 1回目だけ実行され、2回目以降は0件。破棄した時刻も書き換わらない
- 破棄の知らせ … 1件
- `notification_events` の `dedupe_key` に重複0件
- 上限2で回すと 2→2→1→0 と減り、持ち越したぶんも最後には片づく

二重実行を防いでいるのは行ロックではなく、
**条件付きの UPDATE（`where status = 'active'`）と `dedupe_key` の一意制約**の2つ。
`for update skip locked` は使っていない。

---

### D185 通知は3層に分け、プッシュに頼らない（P5 追加）
...
| 表 | 役目 |
|---|---|
| `notification_events` | 何が起きたか。1件1行。**ここが真実** |
| `notification_deliveries` | どの経路で送ったか・送れたか |
| `push_subscriptions` | ブラウザのプッシュの宛先 |

**プッシュが届かなくても、出来事は消えない。**許可していない人・
取り消した人・端末を変えた人・対応していないブラウザの人にも、
次にサイトを開いたときに画面へ出る。

#### 同じことを二度知らせない

`dedupe_key` を unique にして、鍵の作り方で「何をもって別の出来事とするか」を決める。

| 種類 | 鍵 | いつまた知らせるか |
|---|---|---|
| 予定超過 | 挑戦ID ＋ そのときの予定終了時刻 | 延長して時刻が変われば、また |
| 24時間放置 | 挑戦ID ＋ 放置の起点 | 制作を再開して起点が動けば、また |
| 自動破棄 | 挑戦ID | 二度と（1回きり） |

#### 送るのは3つだけ

制作予定時間の超過・24時間の放置・48時間の自動破棄。
**通知にお題の語や答えは1文字も入らない**（通知はロック画面にも出るため）。

延長の成功はプッシュを送らない。押したその場で画面に出るので、
履歴には残すが割り込ませない。
...
**サイトを開いた直後に窓を出さない。**理由が分からないまま断られると、
戻すのに利用者がブラウザの設定を開く必要が出る。
案内を出すのは制作を始めた場面（お題のページ・ドラフトのページ）だけ。
断られても、制作・投稿・延長はすべて動く。

#### 外部の業者を使わない

プッシュはブラウザの製造元（Google / Apple / Mozilla）の中継所へ
直接 HTTP で投げる。要るのは自分のサイトの署名鍵（VAPID）だけ。
暗号化（RFC 8291 / RFC 8188）は `src/features/notify/push.ts` に書いた。

**本番の鍵は設定していない。**環境変数名と作り方は
`docs/web-push-setup.md`、鍵を作る道具は `scripts/gen-vapid.mjs`。
未設定のときはプッシュだけが送られず、知らせは画面内に出る。

---

### D186 実制作時間は自己申告をやめ、計測値だけを使う（P5 追加）

決定日 2026-09-09。ユーザー確定。

投稿画面にあった「実制作時間」の選択欄（10分／30分／1時間…）を外した。
利用者が任意の値を選んで記録を書き換えられる形をやめる。

**画面から欄を消すだけでは足りない。**フォームを直接叩けば値を送れる。
`works` への BEFORE INSERT / UPDATE トリガーが、送られてきた値を捨てて
計測値（お題の開始から投稿までの実経過）で必ず上書きする。
投稿後の書き換えも受け付けない。
...
| 実経過 | `prompts.started_at` と投稿時刻の差 |

投稿画面はこの3つを `get_production_time` で読んで**表示するだけ**で、
送信もしない。

---

## 2026-09-10（検査）本番向けの検査が、新しい画面に追いついていなかった

### 何が起きていたか

本番へ切り替えたあと、本番向けの検査5本のうち3本が落ちた。
落ちかたはどれも「クイズが0問」「使える語が0語」で、
**本番の機能が壊れたように見えた。**実際には機能は動いていて、
壊れていたのは検査の数えかたのほうだった。

原因は3つある。どれも「画面が変わったのに、検査が古い形を探していた」。

| 何を探していたか | いまどうなっているか |
|---|---|
| `name="vocabId"` の欄を数えて「使える語」とする | P5-A で語はボタンになった。送るのは `vocabIds` と `breaks` の2つ |
| `fieldset[data-question]` を数えて「出題数」とする | P1 で1セクションずつ進む形になった。全部並ぶ古い形は `<noscript>` の中 |
| 「これに決める」を押した時間を測る | P0 でそのボタンが消えた。決めるのは伏せカードの1押し |

### なぜ気づきにくかったか

`<noscript>` の中身は、JavaScript が動くブラウザでは**要素として読まれない。**
だからブラウザから数えると 0 問になる。通信で受け取った HTML には入っている。
同じ画面を、ブラウザで見るか通信で見るかで数が変わる。

### どう直したか

- 数と選択肢は**通信で受け取った HTML から読む**（全部そろっているのはこちら）
- いま見えている段の数は**ブラウザから読む**（`data-section-total`）
- 両方を突き合わせる項目を足した。食い違えば、どちらかが違うと言える
- 押し方そのものは、それを本題にしている検査だけが見る。
  知らせ・サブ指令の検査は通信で1回答えて、本題へ進む

### 同意の関門は、通信だけでは見分けられない

`/play` `/saves` `/works` には `loading.tsx` がある。少しずつ送りながら描くので、
途中で送り先を変えられない。Next.js は行き先を本文の中に混ぜて渡し、ブラウザが動く。
**通信の応答は 200 のまま。**一度これを「関門が効いていない」と読み違えた。
関門の検査は、この3つだけブラウザで見る。

### 消せない検査用の利用者が119人残った（原因は認証サービスの中）

道具を直したあとも、119人だけ消えなかった。返ってくるのは次のもの。
...
こちらの表の作りではない。同じ相手を SQL で
`delete from auth.users where id = …` すると**普通に消える**
（トランザクションの中で試して戻した。2026-09-10 の実測）。
外部キーはすべて CASCADE か SET NULL で、SET NULL の列はどれも空を許す。
引き金も削除時のものは無い。

つまり止まっているのは、認証サービスが利用者を消すときに行う
**その他の処理のどこか。**こちらから直せる場所ではないので、
SQL で消すという抜け道は取らず、残数を記録して止める。

残っているのは、過去の実行が作った使い捨ての利用者
（`artist` 44人・`report-artist` 31人・`stayer` 29人ほか）。
...
本番へ向かえるのは `run-production-smoke.mjs` から起動されたときだけ。
片づけもスモークの1つ（`fixtures`）として置き直した。**本番への道は1本のまま。**

道具そのものにも2つ穴があった。どちらも実測で見つけた。

- 1ページ読んでその場で消していたので、消すと後ろが詰まり、
  ページの境目にいた人が飛ばされる（317人が対象で133人しか消えなかった）。
  先に全部並べてから消す形に直した
- 消せなかったときの理由を握りつぶしていた。認証側が 500 を返していたのが
  見えず、「消えた」と「消えなかった」の区別が付かなかった。
  間を空けて数回やり直し、それでも駄目なら理由を出す形にした

---

## 2026-09-10（実装）盤面の読み出し口を、先に本番へ入った作業線と合流させる

### 何が起きていたか

同じ名前・同じ引数の関数 `draft_state_json` を、2つの作業線がそれぞれ作り直していた。

| 作業線 | 何を返すように変えたか | いまどこにあるか |
|---|---|---|
| P0（`20260907120000`） | 引き直し・捨て札・一巡の状態 | この作業木。本番未適用 |
| D191 / D193（`20260909200000` / `20260910140000`） | 形状アシストとサブ指令 | **本番に適用済み** |

後から当てたほうが勝つ。P0 をそのまま本番へ当てると、先に入っている
形状アシストとサブ指令の鍵が返らなくなる。

出所: **2026-09-10 の実測**。本番と同じ並び（関数144本・表52個。本番の実測と一致）を
手元の PGlite に作り、そこへ P0 だけを当てたところ、`draft_state_json` の定義文から
`shape_assist_key` と `sub_directive_key` が消えた。

### どう直したか

`20260910190000_merge_draft_state_contract.sql` を1本足した。
`draft_state_json` を1本だけ作り直す。中身は「P0 が返すもの全部」と
「先に入っていたほうが返していたもの全部」を合わせたもの。どちらの鍵も落とさない。

表も列も1つも作らない。もう片方の migration を持ち込むこともしない。
足した2つの鍵は、行をまるごと jsonb にしてから鍵で引く形（`to_jsonb(ds) -> 'shape_assist_key'`）
にしてある。だから列がまだ無い並び（手元の検査用データベース）でも落ちず、
その鍵は null になる。列がある並び（いまの本番）では、これまでどおりの値が返る。

当てる位置は P0 の直後。P0 より後の5本は `draft_state_json` を作り直さないので、
この1本が最後の定義になる。

### 消えた鍵は0件

本番と同じ並びのデータベースを2つ作り、片方だけに7本を当てて、
同じ手順で盤面を1つ作って `draft_state_json` を呼び、返る JSON の鍵を突き合わせた。

| どこ | 先に入っていたほう | 合流後 | 消えた鍵 | 増えた鍵 |
|---|---|---|---|---|
| てっぺん | 19個 | 20個 | 0個 | `initial_pass_done` |
| 枠ごと | 11個 | 14個 | 0個 | `redo_used` / `needs_pick` / `can_redo` |
| カードごと | 6個 | 7個 | 0個 | `is_discarded` |

値も一致した。`shape_assist_key` は `"box_first"` を入れて `"box_first"` が返り、
枠へ入れた `sub_directive_key` も同じ値が返った。引き直しを1回はさんでも両方残った。

`is_ready_to_complete` は、2つの版で計算の仕方が違う（先に入っていたほうは
「選んだ数と枠の数が同じか」、P0 は「まだ決まっていない枠が0か」）。
実際に始められるモードは `normal` と `hard` の2つで、どちらも
枠の行を持つ作り（`uses_two_stage = true`）なので、両方の版で同じ値になった。
開始直後はどちらも `false`、全枠を確定したあとはどちらも `true`。
合流後は P0 の計算の仕方を採る（P0 の仕様として既に決まっているため）。

### 版番号の衝突（P0〜P5 とは別の問題。まだ直していない）

`20260909180000` と `20260909200000` を、2つの作業線が別々の SQL に使っていた。

| 版番号 | 手元の main | もう片方 | 本番に入っているのは |
|---|---|---|---|
| `20260909180000` | `billing_founding_creator_v0.sql` | `profile_rpc_revoke_anon.sql` | もう片方 |
| `20260909200000` | `legal_v2_billing.sql` | `shape_assist.sql` | もう片方 |

本番の履歴表はこの2つを「適用済み」として記録している。だから課金の2本は、
**この版番号のままでは本番へ当てられない。**当てても履歴には既に記録があるので、
`migration repair` で辻褄を合わせることもできない（履歴に嘘を書くことになる）。

将来この課金を本番へ入れるときは、課金側の2本を未使用の版番号へ振り直したうえで、
本番の実態に対して当て直しを検証する必要がある。**今回は課金の2本に触れていない。**

出所: **2026-09-10 の実測**。本番に課金の表・関数が0件で、
...
#### 分類と、直さない判断（2026-09-12）

**分類は「本番のスキーマは正しいが、履歴の文の情報も別の migration を指している」。**
名前だけの食い違いではない。根拠は次の5つ、すべて 2026-09-12 の実測。

1. 履歴 `20260909180000` の行は name が `billing_founding_creator_v0`、statements が106文。
2. 本番に課金の実体は0件。表5つ（`billing_offers` `billing_customers` `billing_purchases`
   `billing_entitlements` `billing_events`）・関数5本（`billing_reserve_slot` ほか）・
   `billing` で始まる型、どれも0。
3. 逆に `profile_rpc_revoke_anon` の効果は入っている。対象の5本について
   `anon` の実行権は全部 false、`authenticated` は4本とも true
   （＝あの migration が入れようとした状態そのもの）。
4. `20260909200000` は name が `shape_assist` で、`draft_sessions.shape_assist_key` も存在する。
   こちらに食い違いは無い。
5. 履歴60行と手元の60ファイルを突き合わせると、違うのはこの1行だけ。
   本番にあって手元に無いのが `20260909180000_billing_founding_creator_v0`、
   手元にあって本番に無いのが `20260909180000_profile_rpc_revoke_anon`。

**この食い違いは、いまのところ何も壊していない。**Supabase CLI は版番号だけで
突き合わせる。実測（2026-09-12、CLI 2.111.0）では
`supabase migration list` が60本すべて Local と Remote の両方に並び、警告は出ない。
`supabase db push --dry-run` は `Remote database is up to date.` を返す。
つまり次の deploy も、migration の比較も、この行のせいで止まることはない。

**だから履歴は書き換えない。**`migration repair --status applied` は履歴の行を
手元の同じ版番号のファイルで置き換える操作で、本番の履歴表そのものを触る。
壊れていないものを、本番へ書き込んで直す理由が無い。直すとすれば、
履歴を監査の正本として使うときか、履歴から新しい環境を作り直すときで、
そのときに「直す前の行・直したあとの行・スキーマが1つも変わらないこと・戻し方」を
先に示してから行う。出所: ユーザー指示「既に本番履歴に関係しているなら、
migration repairを勝手に行わず停止して報告」。

**課金の2本は、未使用の版番号へ振り直した**（2026-09-12、枝 `billing-renumber`）。
`20260909180000` → `20260912140000`、`20260909200000` → `20260912150000`。
SQL の本文は1バイトも変えていない（改名の前後で sha256 が同じことを確かめた。
`997f9e9b…` と `d9711e75…`）。新しい番号は、本番の履歴の最大 `20260911120000` よりも、
着地待ちの作業線が使う `20260912110000`〜`20260912130000` よりも後ろにある。
手元の `main`（6d24ec1、未 push）は1バイトも動かしていない。枝を作っただけ。

**追記（2026-09-17）。課金を main へ取り込み、版番号を `20260917090000` / `20260917100000` へ移した。**
9/16 に本番へ `20260914120000` が入り、`20260912140000` / `20260912150000` は本番の最大より
前の番号になった。この位置のままでは `npm run db:deploy` で当てられない（CLI が最後より前への
挿入を断る。`--include-all` は使わない決まり）ので、本番の最大より後ろへもう一度移した。
SQL の本文は変えていない（sha256 は同じ）。課金の決定のうち「売れる形にする」は、main の
D188（検索）と番号が重なっていたので D201 へ移した（D201 の節の注を見ること）。
...
### D194. 回答に「何が答えたか」を持たせる。人間の回答だけが数字を動かす

出所: ユーザー指示（2026-09-10）「answersには、『誰が答えたか』ではなく
『何によって生成された回答か』を表す種別を持たせる。初期値は2種類だけ：
human / system。」

#### 何のためか

将来、新規利用者の最初の作品へ「システム回答」を付けたい。
最初の一周（お題を受け取る → 描く → どう見えたか答えられる → 結果を見る）を
他人任せにせず成立させるためである。

ところが回答をそのまま入れると、伝達率・ビタ当て・2択当て・順位・配給・
回答者の成績が、人以外の回答で動いてしまう。このサービスの数字は
「他の人にどう見えたか」だけを意味しているので、そこが崩れる。

だから**回答を1件も作る前に、区別する土台を先に入れた。**
この決定の範囲では、システム回答はまだ1件も作られない。

#### 「誰が」ではなく「何が」

足した列は、回答した人を指すものではない。その回答が何によって作られたかを指す。
だから名前も actor ではなく source にしてある
（`answers.answer_source`。既にある `answer_mode` の隣に並ぶ）。

固有名（bot / tutorial）にしなかったのは、用途を1つに狭めないため。
出所: ユーザー指示（2026-09-10）「勝手に bot / tutorial 固有名へ狭めない。」

#### 利用者はシステムを名乗れない

守り方は画面ではない。**名乗る口が1つも無い。**

- 回答を出す窓口（`submit_answer`）は、この列を引数に取らない。
  列を書かずに insert するので、既定の human が必ず入る
- 回答の表そのものへの権限は、未サインインにもサインイン済みにも0件（実測）
- あとから種別を書き換えることも、引き金が断る

3つめは利用者向けの守りではない。集計は入れたときに1回だけ数えるので、
...
既にある UNIQUE(work_id, user_id) は、user_id が空だと効かない
（Postgres は空同士を重複と見なさない）。システム回答の持ち主を将来どう決めるかに
関係なく効くよう、種別で絞った索引を別に置いた。
**人間の側の一意制約には触れていない。**

#### 何を人間だけで数えるか

集計は入り口の4つの引き金で止める。そこで止めれば、順位（`get_rankings`）は
枠ごとの集計を読むだけなので、あの関数に触れずに外れる。

止めた先は、作品の回答数・枠ごとの集計・ヒント別の集計・回答者の成績。
配給（D169）の band 判定と、作者へ返す集計と、回答履歴3本も人間だけにした。

配給については、システム回答が付いただけで「回答0件」から外れてはいけない。
まだ誰にも読まれていないからである。
出所: ユーザー指示（2026-09-10）「human回答0件 + system回答1件 → band1。」

#### まだ作っていないもの

出所: ユーザー指示（2026-09-10）「まだ作らない：system answer queue / AI API /
generation RPC / onboarding state / profiles列 / notification文言 /
resultのsystem専用表示 / tutorial route / tutorial prompt / rate limit」。

知らせ（D192）の取り決めも変えていない。システム回答がまだ生まれないので、
先回りして文言を変える理由が無い。

#### 版番号の選び方

別の作業線が同じ時間帯に番号を取っている。全作業木6つ・全ブランチ・
本番の履歴を数えて、使われている最大は 20260910190000 だった
（別作業線の未コミット分を含む。実測 2026-09-10）。それより後ろから取った。

### D195. システム回答は、信頼されたサーバーだけが積んで・取り出して・保存する

出所: ユーザー指示（2026-09-10）「『システム回答を安全に生成・保存できる内部基盤と
待ち行列』を作る。まだ外部AIには接続しない。まだ投稿時に自動enqueueしない。
まだ利用者向けオンボーディングは有効化しない。」

#### 何のためか

新規利用者の最初の作品に、誰からも回答が来ないまま終わる時間を作らない。
そのために仕組みが1件だけ答える。D194 でその回答を区別する土台は入っているので、
ここで作るのは**その回答を作れる唯一の経路**である。

#### v1 の対象は prompt_first だけ

初回体験は「お題を受け取る → 描く → 回答される → 結果を見る」の一周であり、
持ち込み（art_first）には最初の「お題を受け取る」が無い。
出所: ユーザー指示（2026-09-10）「v1の対象：prompt_firstのみ。art_firstは対象外。」

判定は `prompts.origin` を見る。実測でこの列は draft / saved / daily / art_first の
4値なので、art_first かどうかだけで分かれる。**新しい列は作っていない。**

#### 待ち行列は、1つの作品につき1行だけ

同じ作品へ何度積んでも行は増えない（`unique (work_id)`）。
再び試すときは、この1行の状態を戻す。
出所: ユーザー指示（2026-09-10）「duplicate enqueueで新しいqueue行を
無制限に増やさない。」

状態は5つ。待っている・処理中・保存できた・失敗した・作る必要が無くなった。
`storage_cleanup_queue` は「消せたら時刻が入る」だけで途中の状態が無いので、
名前の付け方と権限の閉じ方だけを合わせ、中身は写していない。

#### この表を「初回体験が済んだ印」として使わない

作品を消すと、この行も一緒に消える（回答が消えるのと同じ）。
だからこの表を進み具合の記録として使うと、作品を1つ消しただけで初心者へ戻る。
出所: ユーザー指示（2026-09-10）「『作品を削除したら初心者へ戻る』挙動にはしない。
queue履歴をオンボーディング状態そのものとして代用しない。」

進み具合をどこに持つかは、この決定では決めていない。

#### 保存の窓口は、正解を供給する場所ではない

受け取るのは「どの問に、どの語を選んだか」だけ。合っているかどうかは
窓口の中で選択肢の表を見て決める。呼ぶ側から合否を渡させない。
正式なお題の語・サブ指令・形状の手がかり・作者の言葉も受け取らないし返さない。
出所: ユーザー指示（2026-09-10）「保存入口は『AIが選んだ選択肢を保存する場所』
であって、正解情報を供給する場所ではない。」

#### 呼ぶ側を信じない

秘密の鍵は行の守りも列の権限も素通りする。だから保存する直前に、
作品が消えていないか・公開されているか・表示を止められていないか・
持ち込みではないか・人が先に答えていないか・すでにシステム回答が無いか・
問がその作品のものか・語がその問の選択肢にあるか・数と重複、を全部見直す。
人の窓口（`submit_answer`）と同じ関門を通している。

#### 人が先に答えたら、システムは作らない

目的は「初回の回答ゼロを無くすこと」で、人が答えたなら目的は済んでいる。
...
`answers.user_id` を空（NULL）にした。仕組みを人の顔として見せないためである。

実測でこの列はもともと空を許し、登録が消えたら空になる外部キーが付いている。
つまり空は既存の形の中にある。人の「1人1回」（UNIQUE(work_id, user_id)）は
空同士を重複と見なさないので邪魔をせず、システムの1件までは D194 の索引が受け持つ。
安全の本体は持ち主ではなく種別（answer_source）の側にある。

#### 2人の働き手が同じ仕事を取らない

待っている行を押さえてから状態を進める（`for update skip locked`）。
押さえられた行は、もう一方からは見えない。
出所: ユーザー指示（2026-09-10）「二つのworkerが同じ行を同時claimできないこと。」

#### 再び試すのは、頼まれたときだけ

失敗した仕事は、戻すまで取り出せない。何度でも勝手に試し続ける形にしていない。
上限の回数はまだ決めていない。
...
外部の AI への接続・provider の道具・画像の解析・投稿時の自動積み込み・
初回体験の進み具合・1日あたりの上限・知らせや結果の画面の変更。
でたらめな回答も決め打ちの回答も、本番の機能としては作っていない。
出所: ユーザー指示（2026-09-10）「Phase 2ではprovider接続禁止。Production機能
として：ランダム回答 / ダミーAI / 固定回答 も作らない。」

#### 版番号の選び方

本番の履歴の最大は 20260910190000 で、これは別作業線が適用済みだった
（実測 2026-09-10。`supabase migration list --linked`）。
手元で使われている最大は D194 の 20260911090000（本番未適用）。
全作業木7つ・ローカル4ブランチ・リモート11ブランチ・本番履歴のどこにも
20260912090000 は無い。

2026-09-12 に 20260912120000 へ振り直した（D198）。

### D196. システム回答は、作者向けの分析と取り込み枠に入れない
...
D194 の「人間の回答だけが数字を動かす」の延長として扱う。

#### なぜ今か

D194 と D195 は、本番より古い土台（profile-on-main）の上で作った。
本番には、その土台に無い機能が8本の migration で入っていた。
そのうち作者向けの分析（20260908120000 / 20260908160000）と
取り込み枠（20260909090000）は、回答の出所を知らない。
本番の59本の上へ D194 / D195 を載せ直したところで、次のことが起きると分かった。

- 自動取り込みが入っている作品では、システム回答が作者の枠を1つ使う
- 作者の分析・回答一覧・掘り下げ、答えた本人の集計に、システム回答が1人分として混ざる

#### どこで止めたか

分析の母集団を決める関数は2本（`analysis_all_answers` / `analysis_advanced_answers`）。
集計・掘り下げ・語ごとの数はこの2本が返した回答だけを読むので、**2本で人間に絞れば
下流の3本は触らずに済む。**母集団を通らずに answers を直接読む4本
（回答一覧・分析から外す操作・枠の状態・答えた本人の集計）には、同じ絞り込みを足した。

取り込み枠は入口（回答が入った瞬間の自動取り込み）と、取り込む回答を選ぶ関数の
2か所で止めた。入口だけだと、システム回答が入った瞬間に「まだ取り込んでいない
古い人間の回答」を1件取り込んでしまう（選ぶ関数は古い順に選ぶため）。

#### 番号と、分析から外す操作

回答一覧の通し番号と外す操作の番号は、同じ数え方でないと別の回答を外してしまう。
**2本とも人間の回答だけに番号を振る。**システム回答には番号が無いので指定できない。
存在しない番号は、今までどおり `ANSWER_NOT_FOUND` で断る。新しい失敗の名前は作らない。
理由: この窓口は番号しか受け取らない（回答の ID を渡す口が無い）。拒否の形は既にある。

#### 触らなかったもの

- 回答の知らせ（D192）: システム回答でも「回答が来た」になる。今の取り決めのまま。
...
`test/db/run.mjs` の SC 群7件。互換 migration を外すと6件が落ち、入れると全部通る。
人間だけの作品は、互換 migration の前後で、読める数字が1つも変わらない
（同じDBへ後から当てる比較と、同じ乱数の種で前後2つのDBを育てる比較の両方）。
PostgreSQL 18（PGlite）と 17.6（本番と同じ版）の両方で流した。

#### 版番号

`20260912100000`。本番の履歴の最大は 20260910210000、全作業木・全ブランチの最大は
20260912090000（D195）。どこにも無いことを 2026-09-11 に確かめた。

2026-09-12 に 20260912130000 へ振り直した（D198）。

---

### D197. D194〜D196 の着地は案A。本番へは1本ずつ、停止条件を確かめながら当てる

#### 決めたこと（ユーザー発言 2026-09-12「次工程は Landing Preparation」）

- 「着地方式は案A」: main を p0-p5-production-integration まで早送りし、
  その上へ onboarding-phase12-integration を merge する
- 「D194〜D196は実装・互換性検証完了」
- 「get_usage_summaryは将来C」
- 「180000 repairはD194〜D196の後」
- 本番へ当てるのは着地後の main の clean checkout から（手順は docs/landing-d194-d196.md の2節）

#### 決めた後に変わった事実（D198 で決着）

同じ発言の「20260911120000は現番号のままcommitしない」は、発言の時点ですでに守れなかった。
別の作業線が 9b451a1 でそのまま commit し、2026-09-11 22:39 JST に本番へ当てていた
（本番の履歴 60本・最大 20260911120000。2026-09-12 に読み取りで確認）。
このため D194（当時 20260911090000）は、本番の最大より古い番号で当てることになった。
ユーザーは 2026-09-12 に、古い番号のまま後から当てる案を採らず、3本とも新しい番号へ振り直すと決めた（D198）。

#### 用意したもの（どれも本番では実行していない）

- 停止条件の判定: `scripts/landing-d194-check.mjs`（読むだけ）と、期待値 `scripts/landing-d194-expected.json`
- 取り消し SQL 3本: `supabase/rollback/2026091{2110000,2120000,2130000}_*_rollback.sql`（D198 で版を振り直した）。
  システム回答0件・待ち行列0行のときだけ動き、D196 → D195 → D194 の順でしか動かない
- 手順書: `docs/landing-d194-d196.md`

#### なぜ1本ずつか

`db push` は3本をまとめて当てる。途中で止まって確かめる場所が無い。
（D198 で3本とも本番の最大より新しい番号にしたので、`db push` でも順に当たる。
それでも1本ずつにするのは、あいだで止まって確かめるため。）
`db:apply:one` と `migration repair --status applied` を1本ずつ使えば、
そのあいだで停止条件を確かめられる。repair は SQL 本体を流さず、
db push と同じ形で履歴を書くことを使い捨ての DB で確かめた。

---

### D198. D194〜D196 の版番号を、本番の最大より後ろの3つへ振り直す

#### 決めたこと（ユーザー発言 2026-09-12「方針を1点変更する」）

- 「D194だけを古い番号 20260911090000 のまま後追い適用する案は採用しない。」
- 「D194〜D196はまだProduction未適用なので、3本とも新しい未使用versionへ振り直す。」
- 目的: 「Production migration historyを実際の適用順とversion順で一致させる。」

| 決定 | 旧 | 新 |
|---|---|---|
| D194 answer_source | 20260911090000 | 20260912110000 |
| D195 system_answer_queue | 20260912090000 | 20260912120000 |
| D196 system_answer_out_of_analysis_and_import | 20260912100000 | 20260912130000 |

#### 何を変えたか

- ファイル名の版番号だけ。SQL の本文は1文字も変えていない（sha256 が旧版と同じ）
- そのため、SQL の中のコメントに書いた古い番号（各ファイル1行目のファイル名、
  D195・D196 の中の「Phase 1（20260911090000）」など）は古いまま残る
- 参照を新しい番号へ直した: test/db/run.mjs（互換 migration の境目）、取り消し SQL 3本
  （ファイル名・順番の案内・repair で指す版）、scripts/landing-d194-expected.json、
  docs/landing-d194-d196.md、supabase/rollback/README.md
- 過去の記録（PROGRESS の古い節、D195・D196 の「版番号」節の本文）は書き換えず、
  振り直したことを書き足した

#### 版番号の選び方（2026-09-12 に再監査）

- 本番の履歴: 60本・最大 20260911120000（読み取り専用で確認）
- ローカルのブランチとリモート追跡の全17 ref、全コミットの履歴: 最大 20260912100000（旧 D196）
- リモート（ls-remote）の全ブランチの先頭は、どれも手元にあり中身を確認した
- 全作業木（未追跡のファイルを含む）: 最大 20260912100000
- その後ろの 20260912110000 / 120000 / 130000 は、どのコミット・どの作業木にも出てこない

#### 変わること

3本とも本番の最大（20260911120000）より新しくなる。当てる順と版の順が一致し、
履歴の最大は1本当てるごとにその版になる（61本 → 62本 → 63本）。
古い番号を後から当てるための `--include-all` は要らなくなる。

---

### D200. D194〜D196 は版番号を変えずに、時計の migration のあとで本番へ当てる

出所: ユーザー指示（2026-09-12）「これ以上versionを変更しない」、
（2026-09-16）「新しいmigrationが12130000より後ろに存在する場合も、それ自体は問題ではない。
実際の依存順と適用済み状態を確認して判断する。」

#### 何が起きていたか

D198 で 3本を「本番の最大より後ろ」へ振り直した（2026-09-12）。そのあと別の作業線が
`20260914120000_start_clock_on_prompt_confirmation` を本番へ当てた。
そのため 2026-09-16 の時点で、3本の版番号は本番の最大より手前になった。

#### 決めたこと

版番号は変えない。本番へは時計のあとに当てる。
本番の履歴では、版の順と当てた順が 20260912110000〜130000 の3行だけ食い違う。

#### そう決めてよい理由（実測）

- 依存: 時計の migration が置き換えるのは `draft_sessions_set_initial_deadline` /
  `get_active_challenge` / `prompts_set_initial_deadline` / `renew_current_challenge` の4関数。
  3本が作る・置き換える部品（関数26・表1・列10ほか）と1つも重ならない
- 使い捨ての PostgreSQL 17.6 で、本番の履歴と同じ順（時計が先）で当てたときの部品の差は、
  版の順（時計があと）で作った旧い期待値と鍵も値も完全に一致した
- CLI（2.111.0）は版番号だけで突き合わせる。3本を当てて履歴へ記録したあと、
  `supabase db push --dry-run --linked` は「Remote database is up to date」
- 手元の試験はファイル名の順に当てるので、時計があとになる。互換試験 F だけが
  「D196 より後ろを全部当てる」前提だったので、D196 の1本に閉じた

#### 結果

2026-09-16T14:43Z〜14:44Z に本番へ1本ずつ当てた。手順と結果は docs/landing-d194-d196.md の11節。
システム回答はまだ1件も作られず、作る道も呼ばれていない（dormant のまま）。

---

> **ここから下の2節（D187 / D201）は、課金の作業線から 2026-09-17 に取り込んだもの。**
> 書かれた日付は 2026-09-09 で、そのときの版番号（`20260909180000` / `20260909200000`）で
> 書いてある。**いまの版番号は `20260917090000` / `20260917100000`**
> （振り直しの経緯は「版番号の衝突」の節と D201 の追記）。


---

### D187 課金 v0。最初の商品「Founding Creator」を3,000円・有効30名で売る土台を作る

決定日 2026-09-09。ユーザー確定。

#### 何を作ったか

Founding Creator を3,000円の買い切りで30名まで売り、買った人に Founder 権と
Founder 番号を渡すまでの一式。**Creator Pro（定期購入）は作っていない。**
決済事業者は Stripe。Stripe Checkout の買い切り（mode=payment）だけを使う。
...
30枠を「払われた人数を数える」だけで守ると、29人売れた状態で3人が同時に
決済したとき32人になる。**払った人に返金を申し出ることになる。**

だから決済ページを作る前に DB で1枠押さえる。

```
購入要求 → 商品の行を for update で押さえる → 枠を数える
        → 足りていれば reserved の行を1つ作る → Stripe の決済ページを作る
```

商品の行を押さえるので、同じ商品への予約はこの1行で順番待ちになる
（既存の `consume_import_capacity` が `work_import_state` でしている形と同じ）。

枠を占める状態は reserved / paid / refund_pending / disputed の4つ。
refunded / expired / void / reversed は数えない。**「返金と失効で枠が戻る」が、
この4つの並びだけで決まる。**

#### 予約を、こちらの時計で解放しない

決済ページは30分で失効させる（Stripe が許す最短）。ただし
`checkout_expires_at` を過ぎたことを理由に枠を戻さない。
払った直後に知らせが遅れることがあり、時計だけで戻すと
「払ったのに枠が無い」が起きる。

戻す根拠は Stripe 側の状態だけ。ふだんは `checkout.session.expired` の知らせ、
利用者がもう一度購入ページから始めたときは、その場で Stripe に
決済ページの状態を問い合わせて、失効していれば戻す。

知らせが止まっているあいだは、枠が押さえられたまま売り切れ側に倒れる。
**30枠を超えて売るより、一時的に売り切れに見えるほうを取る。**

#### Founder 番号の出し方

要件は4つ。重複しない／決済の確定順／ふつうの失敗で欠番を作らない／
返金した番号は再利用しない。

採ったのは「商品の行を `for update` で押さえる ＋ `max(founder_number)+1` ＋
(offer_id, founder_number) の一意制約」の3枚重ね。

| 案 | 採らなかった理由 |
|---|---|
| 連番の仕掛け（sequence） | 取引が巻き戻っても番号が戻らない。払ってもいない欠番が並ぶ |
| 裸の `max()+1` | 同時に2つ走ると同じ番号になる |
| 採用（錠＋`max()+1`＋一意制約） | 錠が順番を作り、すり抜けても一意制約が2つ目を弾く。弾かれた側は取引ごと巻き戻るので、権限が半分だけ付いた行が残らない |

`max()` は返金済みの行も数える。だから返金した番号は欠番のまま残る。
一意なのは商品ごと（`offer_id` との組）。全体で一意にすると、
2つ目の商品を出した瞬間にぶつかる。

#### 商品名と権限を分ける

判定に使うのは商品名ではなく権限の鍵（`entitlement_key`）。
Founder を買うと `founding_creator` と `beta_access` の2つが付く。
表示のための鍵（バッジ）は作らない。**プロフィールの表示は
「権限が生きているか ＋ 本人の公開設定」から作る。**

商品が増えても判定の場所は増えない。Creator Pro を足すときも、
既存の判定を書き換えずに鍵を1つ増やすだけで済む。

#### 権限を付けるのは Webhook だけ

利用者のブラウザが成功画面へ戻ってきても、それは何の根拠にもならない
（URL は手で打てる）。付けるのは Stripe の知らせだけ。

知らせを受けたあと、権限を付ける前に6つ突き合わせる。
売り方が買い切りか／支払いが確定しているか／決済ページの ID が記録と一致するか／
商品が Founding Creator か／金額／通貨。
**金額と通貨は、DB の商品定義と比べる。**Stripe から来た値どうしでは比べない。

同じ知らせが2回来ても番号は増えない。知らせの ID を主キーにした表に入れ、
2度目の insert が通らなければ処理しない。ただし前回が処理し終えていなければ
（処理済みの時刻が空なら）もう一度処理する。**途中で落ちた知らせを捨てない。**

#### 返金

始まっただけでは取り上げない（`paid` → `refund_pending`）。
Stripe の返金は失敗しうる（status は pending / requires_action / succeeded /
failed / canceled）。成立（succeeded）で初めて `refunded` にし、
権限に取り消しの時刻を入れ、枠を1つ戻し、一覧を欠番にする。
失敗・取り消しなら `paid` へ戻す。

不審請求の申し立ては、途中は `disputed`（枠は押さえたまま・権利も残る）。
利用者側の勝ち（lost）で `reversed` にして取り上げ、
運営側の勝ち（won / warning_closed / prevented）で `paid` へ戻す。
...
#002 匿名希望   本人が非公開のまま、または退会して名前が引けない
#003 欠番       返金・申し立てで取り消し
```

**匿名希望の人を一覧から消さない。**消すと番号が飛び、欠番と見分けが付かない。
欠番と匿名希望は別の事実なので、同じ言葉にしない。
購入直後の既定は非公開。

一覧を作るために課金の表をブラウザへ配らない。返すのは番号と、
出してよい表示名だけ。金額・決済 ID・利用者 ID は関数が返さない。

#### 遮断の作り方は既存と同じ

課金の5表（`billing_offers` / `billing_customers` / `billing_purchases` /
`billing_entitlements` / `billing_webhook_events`）はすべて RLS を有効にして
ポリシーを1本も作らず、`anon` と `authenticated` から全権限を取り上げてある。
既存の17表と同じ形（これで22表）。

書き込みの RPC は11本とも `service_role` にしか渡していない。
利用者から呼べるのは、残り枠・一覧・自分の番号・自分の権限・公開設定の5本だけ。
公開設定の切り替えは引数が1つ（出すか出さないか）しかなく、
**誰の設定かは `auth.uid()` が決める。**他人を指す方法が構造として無い。

#### 既存の関数を1本も書き換えていない

プロフィールの Founder 表示は `get_public_profile` に足さず、
`get_founder_badge` という別の1本にした。あの関数は作品・回答・統計を
1本で返す既存の中心で、課金の都合で書き換えると、課金を戻すときに
あの関数まで戻すことになる。画面の側で2つを並べる。

#### Stripe の公式ライブラリを入れていない

使う呼び出しは4つだけ（顧客を作る／決済ページを作る・読む・失効させる）。
どれも決まった形の文字列を送って JSON を受け取るだけなので、
`fetch` と `node:crypto` で書いた。プッシュ通知を自前で書いたのと同じ判断。

署名の確かめ方は Stripe の公開手順（2026-09-09 に取得）どおり。
`t=…,v1=…` を `,` と `=` で分解し、「時刻 ＋ `.` ＋ 本文そのもの」の
HMAC-SHA256 を鍵で計算して、一定時間で突き合わせる。v1 以外の仕組みは無視する。
時刻の許容は5分。**署名を確かめる前に本文を JSON として読まない。**

#### まだやっていないこと（販売開始の前に要る）

- 規約の改定。いまの本文は「本サービスの利用は無料です」と明記している
  （`supabase/migrations/20260808200000_legal_v1.sql:218`）
- プライバシーポリシーの改定（決済事業者への提供）
- 特定商取引法に基づく表示の画面
- 本番の Stripe 設定（商品・鍵・受け口の登録・購読する知らせの選択）
- `billing_offers.is_active` を true にすること。**行を足しただけでは売れない**

---

### D201 課金 v0 を「売れる形」にする。法務・同時実行の実測・Stripe の版の固定

> **番号について（2026-09-17 に振り直した）。**この節は課金の作業線（`6d24ec1`）で
> D188 を名乗っていたが、main では同じ D188 を「検索」の決定が先に使っていた。
> 課金を main へ取り込むときに、どの作業線でも未使用の D201 へ移した。
> コード・SQL・試験に「D188」の記述は1つも無い（2026-09-17 の実測、`git grep`）ので、
> 動かしたのはこの文書の番号だけ。**課金の commit の説明文に残る「D188」はこの節を指す。**

決定日 2026-09-09。ユーザー確定（D187 の続き。**新しい機能は足さない。**
売る前に済ませておくことだけを片づける回）。

#### 1. 規約の「無料です」を書き換えた

改定前の本文は「**本サービスの利用は無料です。**」と明記し、
限定公開の期間中は「有料の機能・課金」を行わないと書いていた
（出所: `supabase/migrations/20260808200000_legal_v1.sql` の第13条）。
Founding Creator を売ると、この一文と正面から食い違う。

第13条を「費用と、有料の商品について」に置き換え、7つに分けた。

| 条 | 書いたこと |
|---|---|
| 13-1 | 無料で使える範囲（**基本機能は制限しない**と明記） |
| 13-2 | 商品の内容・価格・販売数・終了の条件・特典 |
| 13-3 | 支払いの方法（**カード情報は運営者が受け取らない**） |
| 13-4 | 返品・返金（7日以内・全額・返金後に何が終わるか・欠番） |
| 13-5 | 譲渡の禁止と、本人確認できたときの結び直し |
| 13-6 | **購入の手続きを始める時点で、いま有効な版への同意が要る** |
| 13-7 | 特定商取引法に基づく表示への導線 |

プライバシーポリシーには、決済で取得する情報（Stripe が発行する識別番号・
支払い成立の事実と日時・金額・通貨）と、**カード情報を保存しないこと**、
Stripe へ情報が渡る瞬間が「購入の手続きを始めたときだけ」であることを足した。

本文の原本は `docs/legal-draft.md` のまま。**migration は手で書かない。**
`scripts/build-legal-migration.mjs` に `--slug` と `--prev` を足して、
2本目以降を `20260909200000_legal_v2_billing.sql`（いまは `20260917100000_legal_v2_billing.sql`）として出せるようにした
（1本目と同じ `legal_v1` という名前で2本並ぶと、どちらが新しいか分からない）。

#### 2. 同意を、お金を受け取る前に取る

規約 13-6 の守りは画面ではなく DB に置いた。`billing_reserve_slot` が、
枠を押さえる前に「いま有効な版へ同意しているか」を見る。
**見方は投稿の門番（`app_guard_works`）とまったく同じ。**
同意していなければ `TERMS_NOT_AGREED` で断る。

画面は `get_founder_offer_status` が返す `agreed` と、いま有効な2つの版を読み、
同意していない人には購入のボタンではなく同意の form を出す。
**form が送るのは版の文字列だけで、誰が同意したかは送らない**
（`auth.uid()` が決める。`agree_to_documents` が版の正しさを見直す）。

#### 3. 特定商取引法に基づく表示は、規約と別の画面にした

規約は「同意してもらうもの」で版ごとに同意の記録が残る。
特商法の表示は「売る側が名乗るもの」で、同意を取る対象ではない。
混ぜると、名乗りを1文字直すたびに全員へ同意を取り直すことになる。

`/tokushoho` を作った。**価格と販売数をこの画面に書かない。**
DB の商品定義から読む。書き写すと、値段を変えた日に表示だけが古くなる。

氏名・住所・電話番号は載せず、「**請求があれば遅滞なく開示する**」と表示した。
特定商取引法は、その旨を広告に表示し実際に遅滞なく提供できる場合、
これらの省略を認めている（消費者庁「通信販売広告Q&A」）。
個人が住所を常時公開する負担を避けるための選択で、
**請求があったときに開示できることが前提。**

#### 4. 本物の Postgres で、同時実行を実測した

PGlite は接続が1本しかなく、2つの取引を同時に走らせられない。
D187 の時点では「順に呼んで片方だけ通る」までしか見ていなかった。

`embedded-postgres`（PostgreSQL 18 の実体を一時フォルダで動かす）を
開発用の依存に足し、`test/db/concurrency.mjs` を書いた。
接続を複数開き、`Promise.allSettled` で本当に同時へ投げる。

| 見たこと | 結果 |
|---|---|
| C1 押さえている間、後から来た予約は待たされる | 合格 |
| C2 残り1枠へ同時に2件 → 通るのは1件 | 合格 |
| C3 弾かれた側に購入・権限・顧客が残らない | 合格 |
| C4 10件を同時に確定 → 番号は 1〜10 で重複なし・失敗0 | 合格 |
| C5 同じ人が2接続から同時に押さえる → 購入の行は1つ | 合格 |
| C6 同じ知らせが2接続から同時 → 押さえるのは1つ | 合格 |

**合格だけでは、試験に歯があるかが分からない。**
`billing_reserve_slot` から `for update` を1語だけ抜いた関数を作って
同じ同時実行を投げたところ、通った予約が2件・枠の使用が **31 / 30** になった。
錠がある状態では1件しか通らない。この差で、試験が錠の有無を
見分けられることを確かめた。

#### 5. Stripe の API の版を、こちらで握る

公式のライブラリを入れていないので、版を指定しないと
**Stripe の口座の既定**が使われる。既定は Stripe の画面から変えられて、
変わった瞬間に返ってくる項目の名前や形が変わる。

`STRIPE_API_VERSION = "2026-08-26.dahlia"` を1か所で決め、3か所で揃える。

1. こちらから呼ぶとき … `Stripe-Version` ヘッダーに載せる
2. 知らせを受けるとき … Stripe 側で受け口を作るときに同じ版を選ぶ。
   届いた知らせの `api_version` を `billing_webhook_events.api_version` に残す
3. 記録 … `.env.example` と、この記録に同じ文字列を書く

**版がずれた知らせを、断らない。**断ると払い終えた人が Founder 権を
もらえないまま止まる。こちらが読む項目（売り方・支払い状態・金額・通貨・
metadata）は版をまたいで名前が変わっていないので処理は続け、
ずれた事実を DB と実行記録の両方へ残して、あとから突き合わせられるようにした。

出所: https://docs.stripe.com/api/versioning （2026-09-09 取得。当時の最新）

#### 6. 課金だけを、HEAD だけの作業木に切り出して確かめた

手元の作業木には別の作業線の変更が113ファイル分ある。
課金がそれらに寄りかかっていないかを、HEAD から作った別の作業木へ
課金の変更だけを移して確かめた。

そこで1つ見つかった。**課金の migration が、別の作業線が作る
`admin_audit_log` を前提にしていた**（購入の結び直しを記録するため）。
当てた瞬間に `relation "public.admin_audit_log" does not exist` で止まる。

直し方は「表があるときだけ CHECK を広げる」。
結び直しを呼んだときに表が無ければ `AUDIT_LOG_MISSING` で断る。
**記録を残せない運営操作を、黙って通さない。**
これで課金だけを先に当てられる形になった。

#### 何を引き換えにしたか

- 開発用の依存が1つ増えた（`embedded-postgres`）。本番の動作には入らない。
  PostgreSQL の実体を落としてくるので、`npm ci` のあと
  `node node_modules/@embedded-postgres/darwin-arm64/scripts/hydrate-symlinks.js`
  が要ることがある（この端末では要った）。
- 規約の版が上がるので、**次に投稿する人・購入する人は同意画面を通る。**
  仕様どおりだが、既存の利用者にも一度出る。
- 画面が2つ増えた（`/tokushoho` と、購入前の同意の form）。
  フッターにも1行増えた。


#### 鍵が渡されたあとに実施する試験（この1本に固定する）

いまは `STRIPE_SECRET_KEY` と `STRIPE_WEBHOOK_SECRET` が無い。
**鍵が無いあいだは、この手順を推測で先取りしない。**
状態の呼び名は `BLOCKED: STRIPE_TEST_CREDENTIALS`。

鍵が渡されたら、下の8段を**この順で1回だけ通す。**
途中で止まったらそこで直し、直したら1段目からやり直す。
（順序を変えない。途中から始めない。段ごとに見るものを決めてある。）

| 段 | やること | 通った証拠として見るもの |
|---|---|---|
| 1 | `/founder` から Checkout を作る | Stripe の決済ページへ移る。`billing_purchases` に `reserved` が1行、`stripe_checkout_session_id` が入っている |
| 2 | 3,000円をテストカードで支払う | Stripe 側で `payment_status = paid`。金額が 3000・通貨が jpy |
| 3 | Webhook が届く | `billing_webhook_events` に `checkout.session.completed` が1行。`api_version` が `2026-08-26.dahlia` |
| 4 | Founder 番号と権限が付く | 購入が `paid`、`founder_number` に数字、`billing_entitlements` に `founding_creator` と `beta_access` の2本 |
| 5 | 公開設定を切り替える | `/founder` で「表示名を出す」に変えると `/founder/members` に名前が出て、戻すと「匿名希望」になる |
| 6 | Stripe の管理画面から全額返金する | 返金の知らせが届き、購入が最終的に `refunded` になる。Stripe の知らせの順や返金の処理によっては、途中で `refund_pending` を通ることがある（2026-09-17 改定。下の注） |
| 7 | 欠番になる | `/founder/members` でその番号が「欠番」。番号は他の人に付け直されていない |
| 8 | 枠が戻る | `get_founder_offer_status` の `used` が1つ減り、`remaining` が1つ増える。同じ人がもう一度買える |

> **6段目の期待を改定した（2026-09-17、ユーザー指示「D201の返金期待値を変更する」）。**
> 旧: 「返金手続き中（`refund_pending`）→ 返金済み（`refunded`）」を必ず通る。
> 新: 最終的に返金済みになる。Stripe のイベント順序・返金処理によっては、途中で返金手続き中を通ることがある。
> 理由: Stripe のテストモードでテストカードを返金すると、`refund.created` の時点で status がすでに succeeded で、
> 購入は paid から直接 refunded になった（2026-09-17 に2回実測。D205）。
> 変えないもの（不変条件）: 成立した返金は最終的に返金済み／権限（entitlement）を取り上げる／使用枠を戻す／
> Founder 番号は欠番になる／Founder 番号を再利用しない。途中で `refund_pending` を通ったときは、その間は権限を残す。

**テスト用の鍵で通したあと、本番の鍵で同じことをやり直さない。**
本番で確かめるのは、決済ページが開くところ（1段目）までにする。
本番の返金試験は、実際にお金が動く。

受け口を作るときは、API の版に `2026-08-26.dahlia` を選ぶ。
購読する知らせは7つだけ（`.env.example` に一覧がある）。
...
### D204. 課金 v0 の本番着地を、適用した作業線とは独立に確かめる（販売停止のまま）

出所: 実測（2026-09-17T08:14Z〜09:05Z）。本番の DB へは何も当てていない。

#### 何が起きていたか

課金 v0 の2本（`20260917090000_billing_founding_creator_v0` / `20260917100000_legal_v2_billing`）は、
別の作業線が 2026-09-17T07:22:19Z〜07:22:28Z に `npm run db:deploy` で本番へ当て、02323c9 / 006d14f を push していた
（006d14f の Production deploy は 08:00:53Z に成功）。この節は、その結果を記録に頼らず測り直したもの。

#### 確かめたこと

- 履歴: 本番67本・最大 20260917100000。67本すべてに同じ版のファイルがあり、手元にだけある版は0本。
  20260909180000 以外の66本は、履歴に残った文がすべて同じ版のファイルに含まれ、名前も一致
- 課金の SQL: sha256 は `997f9e9b…` / `d9711e75…`。6d24ec1（手元 main）・c92b333（billing-renumber）・78fd292・origin/main で同じ。
  版番号だけが 20260909180000/200000 → 20260912140000/150000 → 20260917090000/100000 と移った
- 使い捨ての PostgreSQL 17.6（UTF8 / ICU、本番と同じ既定の権限）に、本番の履歴の順で65本 → 課金1本目 → 2本目を当てた（47項目合格）
  - 1本目: 足された部品86個はすべて課金の名前（表5・関数20・索引16・起動3・ポリシー0ほか）。消えた部品0。
    変わった既存の部品は `admin_audit_log` の制約3つ（`reassign_purchase` を受け付けるため）だけ。既存の全表と auth.users の行指紋は同じ
  - 2本目: 部品の増減0。行が変わったのは `terms_versions` と `privacy_versions` だけ。有効な版が 2026-08-08 → 2026-09-09
  - 両段とも: 未サインイン・登録利用者・ゲストは課金の5表を読めず、権限の行を作れず、販売を開けない。
    運営用の課金関数13本は anon / authenticated から実行できない。運営の鍵で予約を呼んでも `OFFER_CLOSED`（枠は消費されない）
  - 最後に、課金の関数20本（持ち主・definer・search_path・権限・定義の md5）、表5つ、索引、起動、制約42、ポリシー、
    規約の版6行、public の表の数69 が、本番の読み取り結果と1文字まで同じ
- 構造検査（db:verify:keychain）: 236項目すべて期待どおり
- 受け口: 手元の next start に偽の `STRIPE_WEBHOOK_SECRET` だけを渡し、署名なし・でたらめな署名・別の鍵・10分前・本文の書き換えがすべて 400、
  扱わない種類の正しい署名は 200（DB へ行かない）、`STRIPE_SECRET_KEY` が空の購入の受け口は 503、別サイトからは 403（8項目）
- 本番の受け口: 購入 503・別サイト 403、知らせ 503（`STRIPE_WEBHOOK_SECRET が未設定です`）、GET 405
- test:all 16工程（縦断424・構造225・アップグレード31・単体99・道具37・柵30・同時実行6・記録の自己試験26・ブラウザ131・スモーク11ほか）

#### 20260909180000 との関係

本番の履歴の 20260909180000 の行は、名前が `billing_founding_creator_v0`、文が106本で、そのうち99本は今の課金の SQL に含まれる
（古い課金の下書き）。ところが本番には、同じ版のファイル `profile_rpc_revoke_anon` の効き目（プロフィールの関数4本が anon から呼べず、
authenticated から呼べる）が入っている。課金の表は 09-17 の適用まで存在せず、商品の行の created_at は 07:22:23Z。
課金の2本はこの版の関数にも表にも触れないので、**課金の適用はこの食い違いに依存しない。**repair はしていない。

#### 販売状態（安全側）

- 商品 `founding_creator_v0`: 3,000円・jpy・買い切り・30枠・`is_active = false`・販売期間なし
- `is_active` を true にするコードは無い（migration の既定値と、試験の中の update だけ）。販売開始は運営が SQL を流す操作
- 購入・権限・顧客・受け取った合図: 0件（開始時・スモーク後・片づけ後の3回）
- Stripe の鍵: 手元・環境変数に無い。本番の Vercel は API が forbidden で読めず、受け口の 503 から「少なくとも STRIPE_WEBHOOK_SECRET は無い」とだけ言える

#### 足したもの

`scripts/smoke-billing.mjs`（`npm run smoke:prod -- billing`）。販売停止中を期待値にして、
/founder・/tokushoho・/founder/members の表示と、購入・知らせの受け口を、未サインインと登録利用者の両方から見る。
Stripe は呼ばない（受け口は設定の確認か販売停止で止まる）。販売を始めたら落ちるので、そのときに期待値を直す。

#### 記録の訂正

先の作業線の記録（PROGRESS・launch-checklist）にあった「有効版が 2026-09-08 → 2026-09-09」は、正しくは 2026-08-08 → 2026-09-09。
本番の terms_versions / privacy_versions は 2026-08-04・2026-08-08・2026-09-09 の3行ずつ。

---

### D205. Stripe の試験は、手元のアプリ・手元の DB・Stripe CLI の転送で通す（案 A）

出所: ユーザー指示（2026-09-17）「過去決定どおり、原則として：ローカル/手元のDB + Stripe test mode で試験する。Productionの商品は開けない。」
「可能なら、固定Production webhookを作る前にStripe CLI forwardingを使う。」

#### 決めたこと

- 8段を通す場所は、`test/e2e/server.mjs` の手元のアプリ（next dev）と、PGlite ＋ Supabase の代わりの API。
  販売を開けるのはこの DB の中の `founding_creator_v0` だけ。本番の DB へは接続しない
- Stripe からの知らせは `stripe listen --latest` が手元の受け口へ転送する。署名の鍵は `stripe listen --print-secret` のもの。
  **Stripe の管理画面に受け口を作らない**
- 鍵は macOS のキーチェーン（`drawing-prompt-quiz-stripe-test-secret-key`）から読む。`.env.local` にも Vercel にも入れない
- 最初の通信の前に、鍵の形（`sk_test_` / `rk_test_`）と本番の痕跡を見る。最初の通信は `GET /v1/balance` で、`livemode` が false でなければ止まる
- 6段目の返金は、管理画面ではなく Stripe の API で作る（Stripe 側にできる Refund と知らせは同じ。管理画面は人の手が要る）

#### 道具

`test/billing/stripe-e2e.mjs`（`npm run test:billing:stripe`）。

- `local`（既定）: Stripe と通信しない。1・2段目は DB の関数で代わりにし、3段目以降の知らせは Stripe と同じ手順で手元が署名して本物の受け口へ送る。
  2026-09-17 に50項目すべて合格（3〜8段、同じ event ID の再送・同じ決済ページの別 event・買い終えた人の再購入、
  返金の途中状態・返金の知らせの重複、#002 が #001 を付け直さない、署名なし・違う鍵・10分前・本文の改変・壊れた JSON がすべて 400、
  金額・通貨・商品・売り方・決済ページの不一致が 500 で権限0、未払いは 200 で権限0、失敗した知らせは処理済みにならない、失効で枠が戻る）
- `stripe`: 1〜8段をテストモードへ実際に通信して通す。受け口の連打（同じ人の同時2回）と残り1枠の同時2人も、受け口ごと確かめる。
  **未実施**（鍵が無い。`BLOCKED: STRIPE_TEST_CREDENTIALS`）。鍵が無い・`sk_live_` の形の鍵では、通信の前に止まることだけ確かめた

#### 鍵が入ったら確かめること（いまは仮説で、判定には使わない）

- 受け口の連打: 1回目が決済ページを作り終える前に2回目が来ると、2回目は同じ購入の ID で決済ページを作り直す。
  そのとき `expires_at` が1回目と違う秒になると、同じ Idempotency-Key で中身の違う要求になり、Stripe がエラーを返す可能性がある。
  購入の行は増えない（DB が止める）ので、起きても二重購入にはならない。`stripe` モードの「連打」で、応答の中身を見て判断する
- 6段目: テストカードの返金はすぐ成立することが多く、`refund_pending` を通らずに `refunded` になる可能性がある。D201 は
  `refund_pending → refunded` を期待として書いているので、違ったら D201 の期待を直すかを判断する

#### 新規発見（課金のコードではなく、ブラウザ試験の誤判定）

2026-09-17 の全回帰試験で、ブラウザ試験 131件中1件（[40/I]「お題の全語が出題され、問数が語数と一致する」）が
「選択肢の外に答えが出ている: 取引」で落ちた。同じ群だけを3回流し直すと3回とも合格。

原因: この試験は、作品の画面から回答欄とスクリプトを除いた本文に、お題の語が文字として含まれていないかを見る。
課金 v0 でサイト共通のフッターに「特定商取引法に基づく表示」が増え、その中に「取引」がある。
種のデータのお題は毎回ランダムに引くので、「取引」が引かれた回だけ誤って落ちる。
実測: 手元のアプリで作品の画面を開き、直す前の数え方では「取引」が含まれる（true）、
サイト共通のフッター（`footer[data-site-nav]`）を除くと含まれない（false）。
...
#### 反映（2026-09-17。ユーザー指示「D201/D205のStripe実通信試験結果を正式反映する」）

- D201 の6段目の期待を改定した（D201 の表と注）。道具も「refund.updated が届くこと」を必須から外し、
  不変条件（refunded・権限の取り消し・枠の戻り・欠番・番号の再利用なし）で判定する。途中の経路は記録だけ残す
- **購入の要求が重なったときに、Stripe の英語のエラー文を利用者へ出さないようにした。**
  - 判定（`src/features/billing/conflict.ts`）: Stripe の失敗のうち、HTTP 409 / `idempotency_key_in_use`（処理中の重なり）と
    HTTP 400 / `idempotency_error`（同じ鍵で中身が違う。決済ページの失効時刻が呼んだ秒で変わるため、重なった2つ目がこれになりうる）だけ。
    2つとも 2026-09-17 にテストモードで形を実測した（顧客を同じ Idempotency-Key で同時に6本作ると5本が409、中身を変えて2回目を送ると400）
  - 受け口（`/api/billing/checkout`）はこのときだけ 409 と `{ error: "処理が重なりました。少し待ってから、もう一度お試しください。", code: "CHECKOUT_BUSY", retry: true }` を返す。
    Stripe の元の状態コード・合図・文はサーバーの記録（console.warn）にだけ残す
  - 変えていないもの: 売り切れ・購入済み・販売停止の 409、二重購入を DB が止めること、前の決済ページを使い回すこと、
    それ以外の想定外の失敗の扱い（`billingError` の unknown はこれまでどおり元の文を返す）
- 試験: 単体2件（重なりの判定が2つの形だけを拾い、カードの拒否・別の409・DB の売り切れなどは拾わない／案内の文に英字が無い）。
  判定を常に false にすると落ちることを確認した。`stripe` モードに、同じ人の同時3回（重なった応答が 409・CHECKOUT_BUSY・
  日本語だけで Stripe の文が本文に無い、サーバーの記録に元の失敗が残る、もう一度押すと同じ決済ページ）と、
  画面でボタン側が重なったとき「処理が重なりました」が出て Stripe の文が出ない・もう一度押すと決済ページへ移る、を足した。
  重なりを再現できなかった回は不合格にする（空振りで合格にしない）
- 結果: `local` 50項目・`stripe` 68項目すべて合格（2026-09-17）。重なりは受け口で1回目に再現（応答 200・409・409）、画面でも再現
- Managed Payments を切る修正はそのまま。Managed Payments を使うかどうかは扱っていない


---

### D206. 履歴 20260909180000 は「前提を確かめる1行の SQL」で直す（2026-09-17T15:27Z に本番で修復済み）

日付: 2026-09-17。出所: ユーザー指示「20260909180000 migration history anomaly の修復方法を使い捨て PostgreSQL で完全再現・比較する。この工程では Production の migration history を変更しない」。
詳細と数字はすべて `docs/history-20260909180000.md`。

#### 分かったこと（実測）

- 食い違いは本番の履歴表の1行の name と statements だけ。表・関数・権限は手元のファイルを当てた DB と同じ
  （部品1,515個・権限378個を比べ、違いは 20260909180000 と無関係な2点だけ）。ほかの66行は CLI が書く値と1文字も違わない
- CLI（2.111.0）は `migration list` / `db push --dry-run` で履歴表の version しか読まない。だから日々の運用は今も止まらない
- 放置したときに壊れるのは、履歴を正本にする操作。`migration fetch` の後の `db push` が止まる。履歴だけから作り直すと
  20260917090000 で失敗し、プロフィールの関数4本が anon から呼べる状態で止まる
- `migration repair --status applied` は、手元のその版のファイルで行を上書きする1文（INSERT … ON CONFLICT DO UPDATE）。
  SQL は実行しない。reverted を先に流す必要は無い。作業木に課金の下書きがあると、終了コード0のまま別の誤った行（111文）を書く
- B（repair）と C（1行の SQL）の直した後の履歴は1文字も同じ。スキーマ・権限・データ・構造検査236は前後で同じ。
  後ろに migration を1本足しても A/B/C すべて正常。B も C も、同じ戻す SQL で直す前の履歴に1文字違わず戻った

#### 決めたこと

- 直す方式は C。`scripts/db-history-0909.mjs fix --yes` が、作業木のファイル・流す SQL の sha256・履歴67行・対象行・
  ほかの66行・CLI の list と dry-run を確かめてから、`supabase/manual/20260909180000_history_fix.sql` を1トランザクションで流す。
  理由: 結果は B と同じで、前提が違えば何も書かずに止まる（通し稽古で確認）。DDL を流さない（B は4つ流す）
- 戻すのは `rollback --yes`（`supabase/manual/20260909180000_history_rollback.sql`）。CLI では戻せない

#### 本番修復（2026-09-17T15:27Z 実施）

出所: ユーザー指示「20260909180000 migration history anomaly を、D206で採用した方式CでProduction修復することを承認する」。

- 方式 C（`scripts/db-history-0909.mjs fix --yes`）。CLI の repair（B）は使っていない
- 変わったのは履歴表の 20260909180000 の1行だけ（`billing_founding_creator_v0` / 106文 → `profile_rpc_revoke_anon` / 11文）
- スキーマ・権限・データの差分0（部品1,515個・権限378個・public 全表の件数とハッシュが前後で同じ）。構造検査は前後とも236/236
- rollback は使っていない。修復後の check は「修復後」・list 67/67・dry-run up to date
- 詳細は `docs/history-20260909180000.md` の冒頭

---

### D207. トップは「引く・描く・答える」の3つを押せる手順にし、主ボタンは「お題を引く」1つにする

日付: 2026-09-18。出所: ユーザー指示「初見の利用者がホームを見た数秒以内に、何をするサービスか／最初に何を押せばいいか／お題を引く → 描く → 他人の絵に答える という基本循環を視覚的に理解できる状態にする」「各ステップそのものを押せるnavigationにする」。
詳細（参照した実在の画面・行き先・幅ごとの組み立て）は `docs/home-first-use.md`。

- 見出しは「あなたの絵、何を描いたか伝わる？」、1文は「引いて、描いて、答える。」（ユーザー指示の第一候補のまま）
- 3つの手順は押せる四角（Kahoot! の上部の行き先の形）。引く → /play、描く → 確定したお題を持つ人はそのお題のページ・持たない人は /play、答える → /works
- 「描く」の行き先はページを静的に配ったまま、表示後に /api/challenge を1回読んで差し替える（トップを人ごとの生成にしない）
- 主ボタンは「お題を引く」、副ボタンは「描かずに、絵に答える」。「描いた絵で試す」は文字の入口へ下げた（行き先は同じ）
- 登録を求める位置は変えていない（投稿の直前）

---

### D208. 640px より狭い画面では、ヘッダーの5つの行き先をメニューへ畳む

日付: 2026-09-18。出所: ユーザー指示「モバイルではヘッダーを常に1段にし、必要な5導線を失わず、初見画面の縦領域をナビゲーションで浪費しないこと」「左：ブランドリンク／右：メニューボタン1個」「フルスクリーンmodalにはしない。画面下から出るbottom sheetにも今回はしない」。
詳細（実測・参照した実在の画面・寸法）は `docs/header-mobile.md`。

- 折り返しは実測で 536px より狭いときに起きていた（320px で3段・157px、390px で2段・109px）。畳む幅は既存の `sm`（640px）を使う。新しい区切りを作らない
- 閉じているときヘッダーに出すのは「つたわるかな」と 44×44 のメニューのボタンだけ。高さ61px の1段
- 開くとヘッダーの直下に画面幅いっぱいの板が出る。5つの行き先を縦に1列、1行48px。全画面を覆わず、本文を押し下げるだけ
- 行き先・並び・URL は変えない（/play・/works・/rankings・/notices・/account）。現在のページを示す印は、もともと無いので付けない
- 行き先は1組しか書かない。同じ `<nav>` を、狭いときは板・広いときは横並びとして使う（同じ文言のリンクを画面の外に重複させない）
- 640px 以上の見た目は座標まで同じ（768px と 1280px で前後の x 座標が一致）
- 知らせ（D192）の読み込み方も、数字を出さないことも変えない。ただし狭い画面では、未確認の太字はメニューを開くまで見えない。ボタン側に印を付けるのは D112 に触れるため今回はしない

---

### D209. 初回利用の進みぐあいは、いまの表だけを読んで数える（記録の仕組みを足さない）

日付: 2026-09-18。出所: ユーザー指示「『トップを改善した結果、利用者が実際にどこまで進んでいるか』を、推測ではなく現在DBの事実から測れる状態にする」「新しいanalyticsサービスを入れない／新しいtracking cookieを入れない／page view trackingを入れない／DB migrationを追加しない／本番データを書き換えない」。
定義・本番の基準値・測れないものの一覧は `docs/funnel-v0.md`。

- 数えるのは `npm run db:funnel`。本番へは読むだけでつなぐ（`begin transaction read only` を張り、rollback で閉じる）。個人情報も利用者の id も標準出力に出さない
- 分母は「DB に現れた人」（＝初めて書き込んだ人）であって、トップを見た人ではない。閲覧を記録していないので「トップ閲覧者の何％」は作らない
- 段は、作る側（お題が先 / 絵が先）・答える側・登録の4つに分ける。人数と率を必ず並べて出す（3 / 7 = 42.9% の形）
- 数え方の SQL は `scripts/funnel-query.mjs` に1本だけ置き、本番を読む道具と使い捨ての DB で試す試験が同じものを使う
- 退会・掃除で消えた人の行動は数に入らない（持ち主の列が null になる）。期間が長いほど母数は小さく出る、と道具自身が毎回出力する
- 匿名のゲストは、検査（スモーク）が作ったものと本物を見分けられない。見分けられるのは電子メールを持つ登録済みの検査用利用者だけ
- 閲覧・押下の記録を足すかどうかは、限定公開を広げて実利用者が入ってから決める。この工程では1つも実装していない

### D210. 本番スモークは、自分が作った人を自分で消す（見分けではなく、名指しで）

日付: 2026-09-18。出所: ユーザー指示「『本番スモークを何回実行しても、終了後のProduction利用者・ファネルデータを汚さない』状態にする」「smoke自身が『今回自分が作った匿名guestのID』を実行中に保持し、finally/cleanupでそのIDだけを削除する」「名前・時刻・行動パターン等から後で推測して削除する方式を通常cleanupの主方式にしない」「削除に失敗した場合：smoke全体を成功扱いにしない」。
残していたもの・見つけかた・保証範囲・本番の実証は `docs/smoke-selfclean.md`。

- 匿名ゲストの ID は、作られたその場で2つの独立した経路から拾う。セッション Cookie の復号と、この実行が投稿した作品への回答行。どちらも「この実行が作った物」しか見ていない（`scripts/_smoke-actors.mjs`）
- 消してよいのは帳面に載った ID だけ。最近作られたゲスト全部・回答が1件だけの人・電子メールが無い人・特定の時刻範囲、といった広い条件では消さない
- 片づけは `finally` に置く。成功でも、失敗でも、時間切れでも走る。`kill -9` のように `finally` 自体が走らない場合は保証しない（30日後に `list_stale_guests` が拾う）
- 片づけに失敗したらスモークを不合格にして終了コード1で終わる。旧来の「片づけの失敗は握りつぶす」（D85）は、人を作る経路には適用しない
- 「消した」ことの証拠は返事ではなく件数。一周の前と片づけの後で8つの表を数え、1件でも違えば不合格（`scripts/_smoke-baseline.mjs`）
- 消す順は、画像 → 人に結び付く記録 → 作品 → お題 → ドラフト → 人。人を先に消すと `on delete set null` で持ち主が外れ、誰の行か辿れなくなる
- 本番の人数を報告するときは「認証アカウント数 / 人の行 / 観測できた行動者」を区別する。原則は3つめ（ファネルの分母）を使う
- D209 で実証済みの既知2件は、18項目の指紋を照合して全部一致したうえで削除した。ほかの匿名ゲストには触っていない
- 直すのは1本のスモークではなく、人が生まれる経路の出口。すべてのスモークが使う共通の入口（`scripts/_smoke-http.mjs` の `finish()`）で、開いた入れ物の Cookie から人を拾って消す
- 固定の検査用利用者（`dpq-fixture-`）だけは消さない。使い回す前提で、毎回作り直すと匿名サインインの回数制限を削るため。本番で流したあとは `npm run smoke:fixtures:purge` を別に流す
- この工程のあとの本番の出発点を「スモーク自己片づけ導入後の基準値」として `docs/smoke-selfclean.md` 10節に置く（2026-09-18T00:10Z 実測。人40／登録済み4／検査用0）。以後の増減はここから測る
- 記録の仕組み（閲覧・押下）は今回も足していない。`usage_events` の CHECK 制約も変えていない

---

### D211 作品一覧を「絵そのものがカード」にする（Pinterest を実測して寄せる）

出所: ユーザー指示（2026-09-18）「今回の画像一覧UIは、独自に自由設計しないこと」
「現在のPinterestを一次参考として可能な限り忠実に再現すること」
「『カードの中に作品画像がある』ではなく『作品画像そのものがカードである』状態にする」。

#### 置き換えたもの

| 前 | いま |
|---|---|
| 決まった高さ（192px）の枠に絵を縮めて入れる | 絵の縦横比そのままで、高さは1枚ごとに違う |
| カードの下にタイトル・作者・部門・完成度を常に出す | 通常の状態では絵だけ。文字は1つも出ない |
| 3列固定（画面幅で 1 / 2 / 3） | 画面の幅から毎回計算する（幅2560で10列まで増える） |
| 「次のページ」を押す | 下まで来たら続きを読む。戻ると元の位置に戻る |
| カードを押すと作品ページ（説明が先） | 未回答はクイズへ直行、回答済みは結果へ直行 |
| いいねは作品ページのボタン | 一覧では**カードを0.5秒長押し** |

#### 数値の出どころ

`docs/RESEARCH/2026-09-18_pinterest-masonry.md`。Pinterest の検索結果の画面を
Chromium で開き、画面の幅を31通りに変えて測った記録。列の数・列の幅・隙間・
端の余白・角の丸み・縦長の上限は、すべてこの実測から写している。
**想像で決めた値は `src/features/feed/layout.ts` に入れていない。**
実測でないもの（狭い画面の境目640px、列の上限12）は、その旨をコードに書いてある。

実測と合わせた結果、幅390で2列・768で3列・1280で5列・1920で8列になり、
1列の幅も Pinterest と同じ（幅1440で267.2px、幅1920で221px）になった。

#### タイトルを一覧に出さない

タイトルはクイズの答えの手がかりになりうる。回答前の一覧では出さない。
画像の代替文（alt）にも入れない。読み上げには、代わりにリンクの説明
（「〈作者〉さんの作品。クイズに答える」）が読まれる。

#### いいねを長押しにした（ハートのボタンを置かない）

一覧に数字もハートも置かない（D112 の続き）。カードのどこでも0.5秒押し続けると
付く。押している間、中央からパステルピンクが外へ広がる。

**スクロールを1ミリも止めない。** `touch-action` も `preventDefault` も使わず、
代わりに「押している座標が、まだそのカードの中にあるか」を毎フレーム見る。
スクロールでカードが逃げれば、指が動いていなくてもそこで失効する。
指の側は touchmove で追うので、スクロール中に同じカードを押し続けていれば継続する。

#### 「興味なし」を新しく作った

`work_uninterests` と `set_work_uninterest`。押した本人の一覧からだけ消える。
作品は公開のままで、他の人の一覧にも作者の画面にも残る。件数も一覧も外へ返さない。
ゲストも使える（通報と同じ扱い。押した瞬間にゲストを発行する）。

#### 既存の入口を1つも壊していない

`get_public_works` は旧5引数版・新6引数版のまま。列を足したかったが、
`create or replace` は返す列を変えられず、drop すると本番でDBを先に更新する
時間帯に一覧が落ちる。**別の名前 `get_feed_works` を新しく作った。**
旧い入口は「興味なし」も見ない（旧い画面の見え方を変えないため）。

#### まだやっていないこと

- 回答が終わったあとに関連作品へ変形する仕様（別作業）
- 触れたときのUIを Pinterest に似せること（サインインしないと測れなかった）
