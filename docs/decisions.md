...
このため D194（20260911090000）は、本番の最大より古い番号で当てることになる。
使い捨ての PostgreSQL 17.6 で、その順番（60本 → D194 → D195 → D196）を再現して通した。
このまま当てるか、D194 の番号を付け替えるかは未決。

#### 用意したもの（どれも本番では実行していない）

- 停止条件の判定: `scripts/landing-d194-check.mjs`（読むだけ）と、期待値 `scripts/landing-d194-expected.json`
- 取り消し SQL 3本: `supabase/rollback/2026091{1090000,2090000,2100000}_*_rollback.sql`。
...
また D194 が本番の最大より古い番号なので、`db push` は `--include-all` を付けないと止まる。
