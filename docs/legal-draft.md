...
**第2節・第3節は DB に投入する法務本文の原本です。**
現在の本文を改定するときは migration を生成し、適用後に現行版を確認します。
以下の初期調査記録には作成当時の状態も残しています。
...
| 14 | 広告・賞金競技を行わない／有料の商品は Founding Creator のみ | 課金 v0（D187）。基本機能は無料のまま | 13 | 4 |
...
**お題を引く・作品を投稿する・クイズに答える・結果を見る・共有する
といった基本の機能は、すべて無料でお使いいただけます。**
有料の商品を購入しなくても、これらが制限されることはありません。
...
本サービスは、最初に支えてくださる方のための商品として
**Founding Creator** を販売します。
...
- プロフィールへの Founder 表示（**表示するかどうかは購入者が選べます。
  購入直後は表示しない設定です**）
- Founder 一覧への掲載（**匿名希望を選べます。
  選んだ場合も番号と並び順はそのまま残ります**）
- Creator Pro（将来提供を予定している機能）の**正式開始後、90日間の利用権**
- 新しい機能の試験版への先行参加
- Founder 向けのアンケート・お話をうかがう場への参加

**Creator Pro の90日間は、購入した日からは数えません。**
Creator Pro が正式に開始した時点で、Founder 権をお持ちで、
まだこの90日を使っていない方に対して開始します。

**Creator Pro の提供時期は確定していません。**提供できなくなった場合は、
第13-4条の返金の対象とは別に、その時点でお知らせします。

### 13-2-b. 自由額の単発支援「つたわるかなを応援する」

サービスの開発・運営を任意で支援できます。**500円から100,000円まで、
1円単位の金額を支援する方が選びます。**初期表示は1,000円です。
一回払いのみで、自動更新はありません。ログインは不要で、何度でも支援できます。

**支援に対する機能上の優遇、特典、バッジ、Founder 資格はありません。**
3,000円以上を支援しても Founding Creator にはなりません。
支援額や支援した方の名前を公開しません。

支援は、サービス運営、開発、UI・デザイン改善、サーバー等の維持に使います。
決済の成立後7日以内であれば、お申し出により全額返金します。
二重請求や運営者の不手際の場合は7日を過ぎても対応します。
お問い合わせは第17条の窓口へお願いします。

### 13-3. お支払いの方法

お支払いは、決済代行事業者 **Stripe**（Stripe, Inc. および
その関連会社）の提供する画面で行っていただきます。

**クレジットカードの番号・有効期限・セキュリティコードを、
運営者が受け取ることはありません。**これらは Stripe が直接受け取ります。
運営者が受け取るのは、支払いが成立したという事実と、
Stripe が発行する識別のための番号だけです。

Founding Creator の価格は運営者側の商品定義から決まります。
自由額支援は入力された金額をサーバーとデータベースで範囲確認してから
Stripe の画面へ渡します。

### 13-4. 返品・返金について

**Founding Creator のご購入から7日以内であれば、お申し出により全額を返金します。**
お申し出は第17条の窓口へご連絡ください。
...
- **Founder 一覧では、その番号は「欠番」として残ります。**
  番号を他の方に付け直すことはありません
- Creator Pro の90日間の利用権が終了します
- 販売枠が1つ戻り、他の方が購入できるようになります

**二重に請求されてしまった場合や、運営者の側の不手際による場合は、
7日を過ぎていても対応します。**この場合は上の7日の制限を適用しません。

デジタルな権利という商品の性質上、上記以外の理由による返金・交換は
お受けできません。
...
アカウントに入れなくなった等の事情がある場合に限り、
**運営者がご本人であると確認できたときは、別のアカウントへ
結び直すことがあります。**この場合も Founder 番号は変わりません。

### 13-6. 購入するときの同意

**購入の手続きを始める時点で、この規約とプライバシーポリシーの
いま有効な版に同意していただく必要があります。**

同意されていない場合、購入の手続きは始まりません。
同意した版は、第11条・プライバシーポリシー第5条のとおり記録します。

### 13-7. 特定商取引法に基づく表示

販売事業者・支払時期・引渡時期・返品の条件などは、
**「特定商取引法に基づく表示」**のページに掲示しています。
...
自由額支援のために Stripe の画面へ入力したメールアドレスは、
本サービスのデータベースへ複製しません。

**クレジットカードの番号・有効期限・セキュリティコードは、
運営者のデータベースに保存されません。**入力先は Stripe の画面で、
運営者はそれらを受け取りません（第4条）。
...
- **クレジットカードの番号・有効期限・セキュリティコード**
  （Stripe の画面で入力していただくもので、運営者は受け取りません）
...
**Stripe へ情報を送るのは、Founder 購入または自由額支援の手続きを始めた瞬間と、
その後の支払い・返金の照合のときだけです。**
決済を始めない方の情報が Stripe へ渡ることはありません。
Stripe は日本国外（米国）に本社を置く事業者で、
**決済に関する情報は日本国外で処理される場合があります。**
Stripe が受け取ったカード情報の取り扱いについては、
Stripe のプライバシーポリシーに従います。
