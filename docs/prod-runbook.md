...
- 課金には進まない。
- **どこから出すか**を先に確かめる。長く使っている作業木は `origin/main` より
  遅れていることがあり（2026-09-18 の実測で主作業木は 86 コミット遅れ・
  未コミット 179 件）、そこから出すと本番がその 86 件ぶん巻き戻る。
  0 と 9 で `npm run preflight:db` ／ `npm run deploy:main` が機械的に確かめる。
  満たさなければ非0で終わり、**その先の操作はどれも始まらない。**
...
|---|---|---|---|---|
| 0 | `npm run preflight:db` | 「条件を満たしています」 | 1件でも満たさない | 何もしていないので戻す先は無い |
