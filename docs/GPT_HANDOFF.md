...
>
> 2026-09-09 訂正: **猶予と「時間切れの失敗」は撤去した（D183）。**
> `deadline_at` は予定終了時刻であって、作りかけを壊す期限ではない。
> 過ぎても失敗にならず、投稿も延長もできる。画面には超過した量が出る。
> 延長は押せる時刻の窓が無くなり、いつでも押せる。新しい予定終了時刻は
> `max(いまの期限, 現在時刻) + 0.75T` で、残っている時間は捨てない。
>
> 作りかけが消える条件は「最終有効操作からの経過」だけになった（D184）。
> 24時間で予告、48時間で自動破棄。予定終了時刻とは別の時計で動く。
