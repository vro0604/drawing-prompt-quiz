...
- 2026-09-17 の改定後（6段目の期待の改定と、購入の要求が重なったときの表示）: `local` 50項目・`stripe` 68項目すべて合格
...
D201 の6段目は「Stripe の管理画面から全額返金する」（期待は「最終的に refunded。途中で refund_pending を通ることがある」、2026-09-17 改定）だが、道具は同じ返金を Stripe の API（`POST /v1/refunds`）で作る。
