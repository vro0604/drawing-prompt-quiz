...
**この層の結果はそのまま信じてよい。**いま 106項目<!--count:ブラウザ試験-->。
...
いま `npm run test:db` が 269項目<!--count:縦断試験-->、
`npm run test:db:upgrade` が 31項目<!--count:アップグレード試験-->、
`npm run db:verify:local` が 179項目<!--count:DB構造の検査-->。
...
- 文書に件数を書くときは、数のうしろに印を付ける
  （`106件<!--count:ブラウザ試験-->`）。`npm run check:docs` が、
