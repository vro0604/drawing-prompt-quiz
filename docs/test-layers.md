...
4. **古い作業木から本番を変えられない**（`npm run test:preflight`）。
   使い捨ての置き場に「遠くの origin」役と「手元の作業木」役を作り、
...
#### 落ちたとき、端末のせいかコードのせいかを分ける（2026-09-10）

2026-09-10 に、ブラウザ全件試験（92件）が6回続けて緑にならなかった。
90/92、90/92、起動失敗、91/92、60/92、81/92。**落ちる項目が毎回入れ替わり、**
回答の保存を1度も通らない群（管理画面・アカウント・持ち込み）まで落ちた。

原因は交換領域（swap。記憶領域が足りないとき、中身を保存装置へ一時的に
追い出す仕組み）の枯渇だった。この端末は記憶領域が 8GB で、試験を始めた
時点で交換領域を 13.0GB 使っていた。そこへ `next dev` と PGlite と
ブラウザを同時に立てたので、出し入れするだけで時間が過ぎた。

**見分けたのは、合格した試験まで一様に遅くなっていたこと。**

| 状態 | 結果 | 1件あたり中央値 | 全体 |
| --- | --- | --- | --- |
| 正常 | 92/92 | 約2.9〜3.2秒 | 約6分 |
| 異常 | 広範囲の時間切れ | 6秒超〜24秒超 | 37〜45分 |

出所: ユーザー指示（2026-09-10）「今回の実測基準」。
交換領域 13GB 超の状態では不安定化を確認済み。

だから `startApp` は、**何も立てる前に**交換領域・平均待ち行列・
同時に走っている重い相手を測り、記録ファイルへ残す（`test/e2e/machine.mjs`）。
終わった後にもう一度測るので、詰まっていたのが最初からなのか、
この試験自身が押し出したのかを後から見分けられる。
1件あたりの中央値・90%点・最長も、合格した試験を混ぜて記録に入る。

失敗が広範囲に出たときは、コードの後戻りと断定する前に、
交換領域の使用量・同時に走っている組み立てや試験・
`next-server` / ブラウザ / PGlite の負荷・1件あたりの中央値を先に見る。
...
**この層の結果はそのまま信じてよい。**いま 184項目<!--count:ブラウザ試験-->。
...
いま `npm run test:db` が 466項目<!--count:縦断試験-->、
`npm run test:db:upgrade` が 31項目<!--count:アップグレード試験-->、
`npm run db:verify:local` が 228項目<!--count:DB構造の検査-->。
...
- 文書に件数を書くときは、数のうしろに印を付ける
  （`184件<!--count:ブラウザ試験-->`）。`npm run check:docs` が、
