...
**この層の結果はそのまま信じてよい。**いま 131項目<!--count:ブラウザ試験-->。
...
いま `npm run test:db` が 386項目<!--count:縦断試験-->、
`npm run test:db:upgrade` が 31項目<!--count:アップグレード試験-->、
`npm run db:verify:local` が 211項目<!--count:DB構造の検査-->。
...
- 文書に件数を書くときは、数のうしろに印を付ける
  （`131件<!--count:ブラウザ試験-->`）。`npm run check:docs` が、
