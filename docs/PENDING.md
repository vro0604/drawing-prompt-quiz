...
- 反映: 2026-08-27 のルーチンで実施。環境変数を空にした `npm run build` が通ることを実測した。
  実施のときに、上の「もともとリクエストごとに描かれている（ビルドの出力でも ƒ）」が
  誤りだったことが分かった。**この2画面はビルドの出力で ○（静的）だった。**
  だからこそビルド中に一度実行されていた。force-dynamic を入れた今は ƒ になっている。
  また、force-dynamic を入れただけでは足りず、同じ理由で /saves も落ちた。
  詳細は PROGRESS.md の 2026-08-27 の項を参照。
