...
| A6 | ~~通報の処理は手動運用。管理画面は作らない~~ → **2026-09-08 の管理 v0 で無効になった。**通報の一覧と詳細（`/admin/reports`）を作り、作品の非表示と通報の処理を専用の操作にした。D177 |
...
制作時間のカウントダウンは、お題を確定した時点から始める。ドラフト中のカード選択には使わない。
...
お題を引く**前**に、作者が「どういう形として描くか」の取っかかりを1つだけ
持てる。**正式なお題ではない。**

- 操作は3つ。使わない／ランダムに1つ／自分で選ぶ
- 1回のお題につき **0個か1個**。複数は選べない
- 候補は9つ（人型・動物型・植物型・怪物型・道具型・建造物型・乗物型・
  景観型・気象型）。一覧と重みは `src/features/shape-assist/types.ts`。
  **`tags` には1行も入れない**（入れるとクイズの誤答として出てしまう）
- ランダムは**始める瞬間に1つへ決める。**引き直しても読み込み直しても変わらない
- 保存先は `draft_sessions.shape_assist_key` の1列だけ。null は「使わない」

次のどれにも使わない。クイズの出題・選択肢・正解・伝達率・
ビタ当て／2択当て・次の作品の配り方（D169）・未踏組み合わせの判定・
全語出題（D165）の語数。

守り方は画面ではなく置き場所。`build_quiz_for_prompt` が読むのは
`prompt_cards` と `card_slots` だけなので、この列は届かない。

見えるのは作者だけ。返すのは `get_my_prompt`（`created_by = auth.uid()` で
絞っている）と `draft_state_json` の2本で、回答者へ渡る
`get_work_detail` / `get_work_quiz` / `get_public_works` には鍵を足していない。

持ち込み（art_first）のお題は `draft_session_id` が空なので、常に「使わない」。
...
### 6-1-2. 描き手（描いた絵を持ち込む。art_first。D172。2026-09-08 実装）

**入口が2つになった。**上の 6-1 は「先にお題があって、それを描く」流れ。
こちらは逆で、**先に絵があり、作者が「この絵で何が伝わるか」を選ぶ。**
投稿より後ろ（出題・回答・採点・集計・開示）は 6-1 とまったく同じ経路を通る。

```
/works/import
 ├ [1] 画像を1枚選ぶ（お題を引かない。promptId も要らない）
 ├ [2] この絵で試したいものを、現行の語彙から3〜6件選ぶ
 │      ・分類を選ぶ → その中を検索して選ぶ（333語を一度に並べない。
        当初 337語と書いていたが、2026-09-09 の数え直しで 333語・10分類）
 │      ・分類ごとの上限は DB が返す（カラーは1件）
 │      ・モーフは必須ではない（感情だけ・色と環境だけ、でもよい）
 │      ・3件に届くまで投稿ボタンは押せない
 ├ [3] タイトル・部門・完成度（6-1 と同じ）
 └ [4] 投稿
      ↓ create_art_first_work が1回で作る
      │   prompts（origin = 'art_first'、制限時間なし）
      │   prompt_cards（選んだ語。選んだ順）
      │   build_quiz_for_prompt（全語出題。D165）
      │   create_work（D27 の6検査はそのまま）
      ↓ /works/[id] へ
```

**制作時間を持たない。**投稿の瞬間にお題ができるので、経過時間はほぼ0秒になる。
測っていない値なので、作品ページの「お題の制作時間」もお題の画面の時計も
出さない。時間別ランキングからも外す（「無制限」にも入れない）。

一覧と「次の作品に答える」では、6-1 の作品と混ぜる。出どころ別のタブ・
フィード・ランキングは作らない。ただし `prompts.origin` は必ず残す。
...
### 12-0-2. プロフィールの拡張（D176・2026-09-08・実装済み／本番未適用）

ポートフォリオの上に、**その人が何を描く人かが分かる欄**を足した。

| 足したもの | 数 | どこに出るか |
|---|---|---|
| プロフィールアイコン | 1人1枚 | `/u/[handle]` の先頭、`/account` の編集欄 |
| 描くのが得意 | 0〜5件 | `/u/[handle]`（0件なら節ごと出さない） |
| 見るのが得意 | 0〜5件 | 同上 |

**得意分野は自己申告。成績ではない。** 実際の正答率・ビタ当て率・投稿数・
よく使うタグとは無関係で、システムが「あなたは狐を見るのが得意です」と
認定することもしない。**次の作品の配り方（D169）にも使わない**
（使うと、回答が「その表現を得意とする人にどう伝わったか」に寄る）。

- 選べる語は、持ち込み（art_first）と同じ範囲。有効な語で、いまお題に出る分類の
  ものだけ（実測 2026-09-08 で10分類333語）。旧語彙は選べない
- 保存先は `profile_specialties (user_id, specialty_type, tag_id, sort_order)`。
  固定列にはしない。同じ種類の中で同じ語は2回入らない。描く側と見る側は独立で、
  同じ語を両方に入れてよい
- 上限5件は、画面・受け口・表のトリガーの3か所が守る
- アイコンは `works` バケットの `<利用者ID>/avatar/<乱数>.<拡張子>`。
  `profiles.avatar_path` に置き場所だけを持ち、直接更新はできない
  （`set_my_avatar` だけが書く）。退会すると、作品画像と同じ列で消える
- 受け取る大きさは 2MiB まで（**暫定。私が設定した数字**）。画面は `next/image` に
  大きさを渡して出すので、原寸をそのまま配らない

アカウント画面は「プロフィール／公開設定／アカウント（サインアウト・退会）」の
3つのまとまりにした。**機能は1つも減らしていない。**

関数は3本（`set_my_avatar` / `set_my_specialties` / `get_my_specialties`）。
`get_public_profile` に `avatar_path` と `specialties` の2つの鍵が増えた
（既存の鍵は1つも消していない）。
...
伝えるのは「まだ結果を見ていない作品がある」ことだけ。
何件来たか、誰が答えたかは伝えない。

数が出るのは、押した先の結果画面（D112「数字は取りに行った人にだけ出す」）。

### 17-2. どこに持っているか

知らせの表は無い。`works.result_seen_at`（作者が最後にその作品の結果を
開いた時刻。null はまだ一度も開いていない）の1列だけを持つ。
...
| 関数 | 返すもの | 呼ぶ場所 |
|---|---|---|
| `has_unseen_results()` | 真偽値だけ。件数は返さない | ヘッダー（全ページ） |
| `list_unseen_result_works(limit)` | 未確認の回答がある自分の作品。作品ごとに1行。件数も回答者も返さない | `/notices` |
| `open_my_work_result(work_id)` | `get_my_work_result` と同じもの。返す前に `result_seen_at` を更新する | `/works/[id]?result=open` |

3本とも `authenticated` だけが呼べる。

### 17-4. 確認済みになるとき

作者本人が、その作品の結果を実際に開いたときだけ。
`/notices` を見ただけでは確認済みにならない。
`/works/[id]` を封を閉じたまま見ても確認済みにならない。

作品ページは、封が開いているときだけ `open_my_work_result` を呼ぶ。
閉じているときは読むだけの `get_my_work_result` を呼ぶ。

### 17-5. 回答の側は変えていない

`submit_answer` には手を入れていない。知らせは回答が入った事実から導く。
途中回答も失敗した回答も `answers` に行を残さないので、知らせにもならない。

持ち込み（art_first）も同じ `submit_answer` を通るので、同じように知らせが出る。

---

## 18. サブ指令（D193）

正式な語1つに添える、描くときの手がかり。正式なお題ではない。
...
| 表 | 列 | いつ入るか |
|---|---|---|
| draft_session_slots | sub_directive_key | カードを決めたとき |
| prompt_cards | sub_directive_key | お題を確定したとき（枠から写す） |

どちらも null は「なし」。形は小文字と下線だけ、31字まで。

### 18-3. 混ざらない仕組み

出題を作る関数が読むのは prompt_cards の tag_id と card_slots だけ。
だから同じ表に列を足しても届かない。画面で隠しているのではない。

診断 A45（回答者向け・出題・配給・順位の12本に出てこないこと）、
A46（列に利用者の権限が0件）、A47（持ち込みに入っていないこと）が毎回数える。

### 18-4. 誰が決めるか

対応表は DB ではなく `src/features/modifier/types.ts` にある。24語 × 3候補 = 72件。
DB は「選ばれた鍵」だけを受け取り、形（小文字と下線、31字まで）を検査して保存する。
どの語にどれが付くかを DB は知らない。

付ける確率は 70%。残りの 30% は何も付かない。
候補が複数あるときは、初期版では等確率で1つ選ぶ。
候補が無い語には、何も付かない。

決める順は2段。まず「付けるかどうか」を引き、付ける場合だけ候補から1つ引く。
2段に分けてあるので、候補が2つの語でも4つの語でも、付かない率は 30% のまま動かない。

### 18-4-1. 利用者が鍵を捏造できない仕組み

**ここが今回いちばん大事なところ。**

サインインの証明書はブラウザから読み取れる（部品の既定が httpOnly: false）。
だから「サインイン済みなら誰でも呼べる窓口」の引数に鍵を置くと、
画面を通さず窓口を直接叩いて、好きな鍵を自分のドラフトへ書けてしまう。
最初の実装は実際にその形だった。

いまはこうしてある。

1. カードを決める窓口（`choose_card`）は、サブ指令を知らない。引数は3つのまま。
2. 鍵を書く窓口は別に1本ある。そこは PUBLIC / anon / authenticated から
   実行権限を取り上げてあり、サーバーだけが持つ秘密の鍵からしか呼べない。
3. その秘密の鍵はブラウザへ1バイトも渡らない。
4. 表そのものへの直接の書き込みも、利用者には配られていない。

つまり利用者の側には、鍵を書き込む口が1つも無い。

### 18-4-2. 秘密の鍵でも、無条件には書かない

秘密の鍵は行ごとの守り（RLS）を素通りする。だから書く窓口の側で、
誰の・どのドラフトの・どの世代の・どの枠の・どの語に対して決めたかを
5つとも突き合わせる。1つでも食い違えば1行も書かない。

これで、サーバーが語を読んでから書きに来るまでの間に引き直しが割り込んでも、
古い語あての鍵が新しい語へ貼られることがない。0行のまま静かに終わる。

カードを決めるのと鍵を書くのは2つの操作になる。後半だけ失敗しても、
正式なお題は正しく決まっており、サブ指令が付かないだけになる。
失敗はサーバーの記録へ残す。

### 18-5. 引き直したとき

世代が進んで枠が作り直されるので、古いサブ指令は正式な語といっしょに消える。
捨てる処理を別に書いていない。

サブ指令だけを引き直す操作は用意していない。

### 18-6. 持ち込み（art_first）

付かない。持ち込みはドラフトを持たず、create_art_first_work が
prompt_cards へ直接入れるため、列は必ず空になる。

### 18-7. 初期の対応表

網羅していない。現行の抽選に出る語は333語あるが、扱うのは24語だけ。

- モーフ 8語（灯台・螺旋階段・時計塔・傘・鳥籠・蒸気機関車・花束・仮面）
- 感情 6語（恐怖・羞恥・嫉妬・孤独・歓喜・諦念）
- 性質 4語（透明・重い・脆い・粘着）
- カラー 6語（赤・白・黒・金・水色・桃）

1語につき3候補、合計72候補。候補の無い語には何も付かない。
数合わせで汎用の言い回しを足さない。

表示文は4〜15字。1語の3候補は、視点・構造・時間・対比のように
別々の軸へ枝分かれさせてあり、同じことの言い換えを3つ並べていない。
...
回答1件ごとに「何によって作られたか」を持つ（`answers.answer_source`）。
値は2つだけ。

- `human` … 人が答えた
- `system` … 仕組みが答えた

これは回答した人を指すものではない。人の識別は `answers.user_id` が持つ。

空は許さない。既定は `human`。既存の回答はすべて `human`。

### 19-2. いまはまだシステム回答が作られない

作る窓口が無い。この節が定めているのは「作られたときに壊れない形」だけ。
生成そのものは次の段で作る。

### 19-3. 利用者はシステムを名乗れない

- 回答を出す窓口（`submit_answer`）は、この列を引数に取らない
- 回答の表そのものへの権限は、未サインインにもサインイン済みにも0件
- 種別はあとから書き換えられない（引き金が断る）

3つめは、集計が「入れたときに1回だけ数える」形だからである。
あとから変えると、数え直されないまま数字が食い違う。

### 19-4. 1作品につきシステム回答は1つまで

種別で絞った索引で守る。人間の側の「1作品1人1回」には触れていない。
...
- ヒント別の集計
- 回答者の成績（全体・枠ごと）
- 作者へ返す結果の集計（回答数・全問外した人の数・問数・的中数・ビタ当て・2択当て・枠ごと）
- 次の作品の配り方（D169）の band 判定
- 回答履歴（自分の分・公開の分）

### 19-6. 配給での扱い

人の回答が0件なら、システム回答が何件あっても band1 のまま。
システム回答が付いただけの作品は、まだ誰にも読まれていない。

### 19-7. まだ決めていないこと

- システム回答の見せ方（結果画面での別表示）
- システム回答でも知らせを出すかどうか
- 生成の経路・時期・対象・失敗時の扱い

いずれも次の段で決める。この節の範囲では、知らせの取り決めも結果画面も
1つも変えていない。


## 20. システム回答の待ち行列（D195）

仕組みが作品に1件だけ答えるための、積む・取り出す・保存する道。
**この節の範囲では、まだ何も自動では動かない。**

### 20-1. 対象

お題を先に受け取って描いた作品（prompt_first）だけ。
持ち込みの作品（art_first）は対象外。判定は `prompts.origin` を見る。

### 20-2. 待ち行列

表は `system_answer_queue`。1つの作品につき行は1つまで。
...
| processing | 取り出して処理中 |
| completed | 保存できた |
| failed | 失敗した（戻せば再び試せる） |
| cancelled | 作る必要が無くなった |

作品を本当に消すと、この行も一緒に消える。
**この表を「その人の初回体験が済んだ印」として使わない。**
使うと、作品を1つ消しただけで初心者へ戻ってしまう。

### 20-3. 誰が呼べるか

積む・取り出す・保存する・失敗の印・取り消し・戻す・様子を読む、の7本とも
信頼されたサーバーだけ。未サインインにもサインイン済みにも権限は0件。
表そのものへの権限も0件で、直接読むことも書くこともできない。

### 20-4. 積まない条件

作品が無い／消されている／公開されていない／表示を止められている／
持ち込みの作品／もう回答がある／すでに積んである。
どれも例外にせず「積まなかった」として返す。

### 20-5. 保存の窓口が受け取るもの

作品と、「どの問に、どの語を選んだか」だけ。

受け取らないもの: 合っているかどうか、正式なお題の語、サブ指令、
形状の手がかり、作者の言葉、選択肢の正解。
合否は窓口の中で選択肢の表を見て決める。

### 20-6. 保存しない条件

作品が消えた／公開されていない／表示を止められた／持ち込み／
人が先に答えていた／すでにシステム回答がある。
どの場合も待ち行列を終わらせ、**作品そのものには触らない。**

人が先に答えていた場合は取り消しで終える。
目的は「初回の回答ゼロを無くすこと」で、人が答えたなら済んでいる。

### 20-7. システム回答の中身

- 答えた人は空（NULL）。仕組みを人の顔として見せない
- ヒントは使っていない（仕組みは作者の言葉を開かない）
- 出所は system。だから伝達率・順位・配給・成績のどれも動かない（19節）

### 20-8. 再び試すこと

失敗した仕事は、戻すまで取り出せない。自動では戻さない。
上限の回数はまだ決めていない。
