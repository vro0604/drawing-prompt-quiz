...
> **2026-09-08 追記。この調査は承認され、実装した。**
> 確定した仕様は `docs/decisions.md` の D172 にある。この文書は
> 「実装前に何が分かっていたか」の記録として、直さずに残す。
>
> **1か所だけ、この調査は間違っていた。**5-6 で「カラーの枠は color_1 の
> 1つだけ」と書いたが、実際は `color_1`〜`color_3` の3つある（実測: 検査用DBの
> `card_slots`）。枠の数だけを上限にすると、持ち込みだけ色を3語選べる形に
> なっていた。実装では上限を「枠の数」と `draw_categories.max_per_prompt` の
> 小さいほうにして、色は1件までにしている（D172 の 2）。
>
> もう1か所、**判断が変わったところがある。**5-4 で「`uses_two_stage` を
> true にするのが検査を1行も直さずに済む唯一の形」と書いたが、
> ユーザー確定3により採用していない。`draft_modes.word_source` を足して、
> 診断の側で3経路を分けた。
>
> **語の数も間違っていた（2026-09-09 に訂正）。**下の「語の選択部品について」で
> 「モーフ127・状態8分類で194・カラー16の337語」と書いたが、**実測は333語・10分類**。
> 数え方: まっさらな検査用DBに migration を全部当て、
> `get_art_first_vocabulary()` の戻り値の分類ごとの語数を足した
> （モーフ126・状態8分類で192・カラー15）。
> **本文の337という数字は消していない。**当時そう見積もっていた記録として残す。
