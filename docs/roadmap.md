...
| 手順 | 内容 |
|---|---|
| **手順2** | `/privacy` に**「（公開前に連絡先を記載します）」がそのまま出ている**。本文も雛形のまま |
| **手順9** | **本番 `/works` が検査用の作品で埋まっている**（D123）。**この項目は手順書に無かった** |

### 分からないもの（外部の管理画面・?）

手順3（Turnstile 本番鍵）／手順5（SMTP・回数制限）／手順7（Site URL）／
手順7-b（メール本文。**4種のうち実測済みは1種だけ**）／手順8（Confirm email）／
手順9（**縦断試験は1周も通していない**）／手順10。

### コード側

**Step 17 まで完了。** 公開手順と独立に作れるものは残っていない。
未コミットの文書が 1,500行超。**最初に落とす。**
...
### ドメインが公開手順の詰まりを3つ同時に外す

| 手順 | 詰まっていたもの | 外れる理由 |
|---|---|---|
| **手順2** | 連絡先が雛形のまま | Cloudflare Email Routing（無料）で `contact@` を私用アドレスへ転送 |
| **手順5** | カスタムSMTP（内蔵メールは1時間2通） | 自ドメインなら SPF/DKIM を立てられる |
| **手順9** | 共有カードの絶対URL | `metadataBase` が `NEXT_PUBLIC_SITE_URL` を見ているので変数1つで揃う |
...
### B. 規約とプライバシーポリシーの本文を書く（5h）

1. `docs/legal-draft.md` から本文を起こす
2. **連絡先は `contact@tsutawarukana.com`**（A で用意済み）
3. サービス名を「つたわるかな」で通す（雛形の表記ゆれを潰す）
4. 同意記録の保存期間（5年）が実際の運用と合っているか読み直す

**書いた日には投入しない。** 法務の文章は書いた直後が必ず粗い。**一晩置く。**

### C. 本文を本番へ入れる（3h）

1. 読み直す（**声に出して読むと抜けが見つかる**）
2. migration で `legal_documents` の新しい版を投入 → `npm run db:deploy`
3. `npm run db:verify:keychain < /dev/null` で **`is_current` が各1件**
4. 本番の `/terms` `/privacy` に新しい本文・版番号・連絡先が出る
...
9. 共有カードの URL が `tsutawarukana.com`
...
- 掃除 Cron では消えない。`smoke:fixtures:purge` でも消えない
- **削除は画面の削除の流れ（`/works/{id}/delete`）を通す**
- **縦断試験のいちばん最後にやる**（試験自体が作品を作るため）
- `/works` に「スモーク」「同時投稿」「ランキング上位」「自作いいねだけ」
  「受け取らないはず」が1件も出ないことを目視する
...
1. `README.md` を書き直す。**何のサービスかを一文で**（調査 ①）
2. 設計の見どころを3つに絞って `decisions.md` へ導線を張る。候補：
...
**127個の決定が文書に残っていること自体が、このプロジェクトのいちばん強い部分。**
伝わらないと存在しないのと同じ。
