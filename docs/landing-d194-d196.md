...
2026-09-12 作成・同日に版番号を振り直した（D198）。
**2026-09-16 に、この手順で3本を本番へ当てた。**結果は 11節。0〜10節は、当てる前に書いた計画の記録として残す
（本数や最大の版は当時の値。2026-09-16 時点で本番は61本・最大 20260914120000 に進んでいたので、11節の値が正しい）。
...
- get_usage_summary（将来 C。システム回答の自動化を始める前に作る）→ 2026-09-17 に完了（D202）
...
**2026-09-16 に実行した（結果は11節）。**各段の `stop/go` は、直前の道具の終了コードで決める。
...
## 11. 本番へ当てた結果（2026-09-16）

### 当てる前に分かったこと（4日のあいだに変わっていたもの）

- origin/main は 6f4521a → 58dce03。時計の開始を「お題の確定のあと」にする migration `20260914120000` が
  本番へ入っていた。本番の履歴は **61本・最大 20260914120000**
- そのため D194〜D196 の3本は、版の順では時計の手前に並ぶが、本番へ入るのは時計のあとになる。
  ユーザー指示（2026-09-12「これ以上versionを変更しない」、2026-09-16「新しいmigrationが12130000より後ろに
  存在する場合も、それ自体は問題ではない」）により、版番号は変えていない。
  時計の migration が置き換える4関数と、3本が置き換える26関数・部品は1つも重ならない（D200）
- 本番の構造検査の既知の4項目（A8 / A21 / A24 / A32）は、別の作業線が 2026-09-13 に直していた。
  当てる直前の構造検査は、main（58dce03）の checkout から **209項目すべて期待どおり**
- 本番の deploy は 58dce03（GitHub の deployment 記録: Production、2026-09-14T18:38Z）

### 着地

- 着地の作業線 `land-0916` を origin/main（58dce03）から作り、2026-09-12 の `landing-merge`（10e3c7b）を merge（17bebe8）
- 文書の衝突5件（PROGRESS / README / decisions / launch-checklist / test-layers）を両側を残して解いた
- 意味の衝突が1件あった: 互換試験 F が「D196 より後ろを全部当てる」書き方で、時計の1本も当てて落ちた。
  当てる範囲を D196 の1本に閉じた（d1d0a06）
- 停止条件の期待値を、本番の今の並び（61本）で作り直した（ca5fcc8）。3本が作る・変える部品は、
  鍵も値も旧い期待値と完全に一致した

### 試験（着地の木）

縦断 386 / 構造 211 / アップグレード 31 / 道具 37 / 単体 86 / 柵 30 / 記録の自己試験 26 / 語彙 652 /
型・lint・配色・build 合格 / ブラウザ 131 / ローカルのスモーク 11 / 文書の件数 15/15。
PostgreSQL 17.6（UTF8 / ICU en-US）でも 縦断 386 / 構造 211 / アップグレード 31。
ブラウザ試験は1回目に131件中1件が時間切れ（端末の交換領域 約12GB 使用中）。その1件は単独で7秒で通り、
全件を流し直して 131/131。

再現（使い捨ての PostgreSQL 17.6、本番と同じ既定の権限、本番の履歴と同じ順で61本 → 3本）: 51項目すべて ○。
apply-one → repair → check を3回、repair は SQL を流さない、db push の計画は「up to date」、
停止条件の負テスト、取り消しの安全条件と順番、取り消し3段（部品と履歴が当てる前と完全一致）。

### 本番（clean checkout 8adffc0 から。時刻は UTC）

| 時刻 | 操作 | 結果 |
|---|---|---|
| 14:43 より前 | `db:verify:keychain`（58dce03 の checkout） | 209項目すべて期待どおり |
| 14:43 より前 | `db:verify:keychain`（着地の木） | 不合格10（表64 / RLS64 / A50〜A54 / A56 / A57 / A62。どれも3本が未適用のため） |
| 14:43 より前 | `check pre` | 0（61本・最大 20260914120000、指紋の鍵 834） |
| 14:43:31 | D194 `db:apply:one` → `repair applied --linked` | 成功 |
| | `check phase1` | 0（62本、回答324件すべて human、指紋一致） |
| 14:44:05 | D195 `db:apply:one` → `repair applied --linked` | 成功 |
| | `check phase2` | 0（63本、待ち行列0行、anon/authenticated は窓口も表も触れない、積む道0） |
| 14:44:50 | D196 `db:apply:one` → `repair applied --linked` | 成功 |
| | `check d196` | 0（64本、人の回答だけの分析・取り込み・集計の値が当てる前と同じ、8本すべて人の回答に絞っている） |
| | `db:verify:keychain`（着地の木） | **222項目すべて期待どおり** |
| | `supabase db push --dry-run --linked` | Remote database is up to date |
| 14:46〜14:57 | スモーク notice / answer（公開中のサイト 58dce03 ＋ 新しい DB） | answer は合格。notice は5項目が落ちた → 下記 |
| 14:58:35 | push（58dce03 → cd6a7d5） | Vercel の Production deploy 成功（GitHub の記録 14:59:22Z） |
| 15:00〜15:08 | verify:launch（18項目）・スモーク draft（64）/ answer / notice | すべて合格 |
| 15:08:54 | `smoke:prod -- fixtures --apply --expect 11` | 検査用の利用者11人を消した（33人に戻った） |
| そのあと | `db:verify:keychain` / `check d196` | 222項目すべて期待どおり / 待ち行列0・human 以外0・積む道0 |

notice の5項目: fab73e3（2026-09-15）で、ヘッダーの未確認の有無は描画のあとにブラウザが読むようになった。
ブラウザ試験は同じコミットで「読み込み済みの印」を待つよう直っていたが、本番スモークは直っておらず、
読み込み前の「無し」を読んでいた。同じ実行で DB 側の知らせの一覧は正しく出ていた。
スモークを同じ待ち方に直して（cd6a7d5）流し直し、合格。D194〜D196 の影響ではない
（3本が変えた部品に has_unseen_results / list_unseen_result_works は無い）。

### 当てたあとの本番

- 履歴 64本・最大 20260914120000。3行の name は answer_source / system_answer_queue /
  system_answer_out_of_analysis_and_import、statements は 21 / 37 / 11 文
- 回答 339件（スモーク分を含む）すべて human。システム回答0件。待ち行列0行
- 公開中の11画面・API（/ /works /rankings /play /notices /account /terms /privacy /works/import
  /api/notices/unseen /api/challenge）は 200。どの応答にも「システム回答」「answer_source」「system_answer」の文字は無い
- 取り消しは使っていない（安全条件は満たしたまま: システム回答0件・待ち行列0行）

### 残したもの

- スモークが作った作品10件（本番の作品は 566 → 576 件）はすべて論理削除（公開0）。検査用の利用者を消したので持ち主は空。回答は残る（集計と一致）
- 2026-09-14 以前から残る、履歴 20260909180000 の行の名前（billing_founding_creator_v0）と実体の食い違いは、この工程では触っていない
